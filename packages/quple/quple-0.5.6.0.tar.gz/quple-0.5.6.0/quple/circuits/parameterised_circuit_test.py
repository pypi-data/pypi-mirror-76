...
class ParameterisedCircuitTest(QuantumCircuit, ABC):
    ...
    def _parse_rotation_blocks(self, rotation_blocks):
        result = ParameterisedCircuitTest._parse_blocks(rotation_blocks)
        ParameterisedCircuitTest._validate_blocks(result, SingleQubitGate, allow_template=False)
        return result
    
    def _parse_entanglement_blocks(self, entanglement_blocks):
        result = ParameterisedCircuitTest._parse_blocks(entanglement_blocks)
        ParameterisedCircuitTest._validate_blocks(result, TwoQubitGate)
        return result
    ...
    def _validate_blocks(blocks, gate_feature=None, allow_template=True) -> None:
        for block in blocks:
            if isinstance(block, TemplateCircuitBlock):
                ...
                n_param = ParameterisedCircuitTest._get_parameter_count(block)
                if not n_param:
                    raise ValueError('not a valid gate operation or'
                                     'circuit block: {}'.format(block))
                dummy_params = tuple(sp.symarray('x',n_param))
                ParameterisedCircuitTest._validate_blocks([block(*dummy_params)], gate_feature)
    ...
    def parameterise_gate(self, gate:Union[cirq.Gate, Callable]):
        if isinstance(gate, cirq.Gate):
            ...
            n_param = ParameterisedCircuitTest._get_parameter_count(gate)
