...
def count_one8(sig,io=None):
    ...
    num[3] = num.c.c&num.c.s&num.s.c
    return num
def count_one1(sig,io=None):
    num = Wire(sig,io=io)
    return num
def count_one2(sig,io=None):
    num = Wire(2,io=io)
    num[0] = sig[0]^sig[1]
    num[1] = sig[0]&sig[1]
    return num
def count_one3(sig,io=None):
    num = Wire(2,io=io)
    num[0] = Wire.full_adder_s(sig[0],sig[1],sig[2])
    num[1] = Wire.full_adder_c(sig[0],sig[1],sig[2])
    return num
def count_one4(sig,io=None):
    num = Wire(3,io=io)
    num.c = Wire(2) # sig.full_adder_c()
    num.c[0] = sig[:2].reduce_and()
    num.c[1] = sig[2:].reduce_and()
    num.s = Wire(2)
    num.s[0] = sig[:2].reduce_xor()
    num.s[1] = sig[2:].reduce_xor()
    num[0] = num.s.reduce_xor()
    num[1] = num.s.reduce_and() | num.c.reduce_xor() 
    num[2] = num.c.reduce_and()
    return num
...
def count_one(sig,io=None):
    if len(sig) in {1,2,3,4,8}:
        return globals()['count_one%d'%len(sig)](sig,io=io)
