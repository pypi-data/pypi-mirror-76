...
def calc_width_x_xy(expr):
    expr.width = expr.lhs.width
    if not expr.rhs._is_constant:expr_match_width(expr.rhs)
def calc_width_x_yx(expr):
    expr.width = expr.rhs.width
    if not expr.lhs._is_constant:expr_match_width(expr.lhs)
...
def calc_width_fetch(expr):
    if expr.lhs.length>1:expr.width = expr.lhs.width
