...
def prop_default(expr):
    for child in expr.childs:
        if not child._is_propagatable:return expr
    return prop_value(expr)
def prop_shift(expr):
    if expr.rhs._is_propagatable:
...
def prop_abelian(prop_func):
    def prop(expr):
        ...
        elif expr.lhs._is_propagatable:
            if expr.rhs._is_propagatable:return prop_value(expr)
            else:return prop_func(expr.rhs,int(expr.lhs),expr)
        else:
            if expr.rhs._is_propagatable:return prop_func(expr.lhs,int(expr.rhs),expr)
            else:return expr
    return prop
...
def prop_nonabelian(prop_l,prop_r):
    def prop(expr):
        if expr.width is None:return prop_default(expr)
            ...
            if expr.rhs._is_propagatable:return prop_r(expr.lhs,int(expr.rhs),expr)
            else:return expr
    return prop
...
expr_value_calc_funcs = {
    'parameter':lambda expr:expr.value,
