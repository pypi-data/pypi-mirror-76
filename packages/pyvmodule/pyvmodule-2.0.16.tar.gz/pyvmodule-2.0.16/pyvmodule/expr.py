...
class Expr(ASTNode):
    ...
    def __len__(self):
        ...
        return self.width
    @property
    def _is_propagatable(self):return False
    @property
    def _is_constant(self):
        if not hasattr(self,'_is_constant_'):
            if not hasattr(self,'childs'):
                self._is_constant_ = False
            else:
                for child in self.childs:
                    if not child._is_constant:
                        self._is_constant_ = False
                else:
                    self._is_constant_ = True
        return self._is_constant_
    @property
    ...
    def __rlshift__(self,lhs):return wrap_expr(lhs)<<self
    def __rrshift__(self,lhs):return wrap_expr(lhs)>>self
    ...
    def _connect_port(self,m,p):
        if p.io!='input':
            if self.typename=='[]' and self.lhs.typename=='wire':
                self.lhs._mark_driven(self.rhs._range_mask)
                return
            raise KeyError('Assigning "%s" to %s port "%s"'%(str(self),p.io,str(p)))
        self._fix_width(p)
