...
class FaustCli:
    ...
    def _default_cli_params(self):
        @click.command()
        @click.option("--brokers", help="Brokers list with kafka protocol e.g kafka://localhost:9092", required=True,
                      type=str)
        @click.option("--input-topics", help="Input topics as list", required=True, type=str)
        @click.option("--output-topic", required=True, type=str)
        @click.option("--error-topic", help="Topic to send deadletter", required=False, type=str)
        @click.option("--error-topic", required=False, type=str)
    ...
    def add_option(self, option_name: str, required: bool, help: str, type: Any, **kwargs):
        options = dict(kwargs)
