...
class Trainer:
    ...
    def _check_attributes(self):
        """
        Checks for correctness of input attributes
        """
        # Type checks
        typecheck_dict = {
        ...
        for attr, v in typecheck_dict.items():
            if self.__dict__[attr] is None:
                continue

            type_to_check = type(self.__dict__[attr])
            if not issubclass(type_to_check, v) and type_to_check is not None:
                raise ValueError('{} must be of type {} but got {}'.format(
                    attr, v, type_to_check))

        # Value checks
        if self.n_dis < 1:
            raise ValueError('n_dis {} must be at least 1.'.format(self.n_dis))

        if self.num_steps < 1:
            raise ValueError('num_steps {} must be at least 1.'.format(
                self.num_steps))
