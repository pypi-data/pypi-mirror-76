...
class Basic(BaseMethod):
    ...
    def make_(self, input_data: DataFrame) -> MethodOutput:
        numeric_part, other_part = DataFrameUtils.decompose_into_parts(
            input_data,
        ...
        return MethodOutput(
            data=DataFrameUtils.concatenate_parts([
                other_part.head(1),
                numeric_part.to_frame().T,
            ]),
            error=None,
        )

    @staticmethod
    def min(input_data: DataFrame) -> DataFrame:
        return input_data.min(numeric_only=True)

    @staticmethod
    def max(input_data: DataFrame) -> DataFrame:
        return input_data.min(numeric_only=True)
    ...
    @staticmethod
    def median(input_data: DataFrame) -> DataFrame:
        return input_data.median(numeric_only=True)

    @staticmethod
    def std(input_data: DataFrame) -> DataFrame:
