...
pyxieEffekseer::pyxieEffekseer()
	...
	, viewport(Effekseer::Vector2D(480.0, 640.0))
{
}
...
void pyxieEffekseer::update(float dt)
{
	/*if (projection_ortho)
	{
		renderer->SetProjectionMatrix(Effekseer::Matrix44().OrthographicRH(viewport.X, viewport.Y, -400, 400.0));
		renderer->SetCameraMatrix(Effekseer::Matrix44().LookAtRH(Vector3D(viewport.X / 2.0f, viewport.Y / 2.0f, 200.0f), Vector3D(viewport.X / 2.0f, viewport.Y / 2.0f, -200.0f), camera_up));
	}
	else*/
	{
