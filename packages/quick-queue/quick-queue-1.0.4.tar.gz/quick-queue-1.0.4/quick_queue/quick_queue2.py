...
def QQueue(*args, **kwargs):
    ...
    :param max_size_bucket_list: (only if sensor is enabled) max size bucket list. If None is infinite.
                                 By defatult: None
    """
    QuickQueueManager.register('QuickQueue', QuickQueue)
    qqm = QuickQueueManager()
    qqm.start()
    return qqm.QuickQueue(*args, **kwargs)


class QuickQueueManager(multiprocessing.managers.SyncManager):
    def __init__(self):
        multiprocessing.managers.BaseManager.__init__(self)


class QuickQueue(multiprocessing.queues.Queue, multiprocessing.managers.SyncManager):
    ...
    def __init__(self,
        ...
        # QuickQueue.register('QuickQueue', QuickQueue)
        # self.start()
        # self.QuickQueue(maxsize=maxsize,
    ...
    def put(self, value, *args, **kwargs):
        ...
        try:
            ...
            self.put_bucket([value], *args, **kwargs)
