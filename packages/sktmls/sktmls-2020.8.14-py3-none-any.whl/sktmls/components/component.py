...
class ComponentClient(MLSClient):
    ...
    def get_component(self, id: int = None, name: str = None) -> Component:
        """
        ...
        components = self.list_components(id=id, name=name)
        if len(components) == 0:
            raise MLSClientError(code=404, msg="컴포넌트가 없습니다.")

        component_id = None
        for component in components:
            if component.is_latest:
                component_id = component.id
                break

        if not component_id:
            raise MLSClientError(code=404, msg="최신 컴포넌트가 없습니다. 모든 컴포넌트를 조회하려면 list_components를 이용하세요.")

        return Component(**self._request(method="GET", url=f"{MLS_COMPONENTS_API_URL}/{component_id}").results)
