...
class KernelShap:
    ...
    """
    def __init__(self, graph, background_data, target=None):
        """
        ...
        Keyword Arguments:
            target {str} -- The name of the output feature in the graph. Specifying None means it will assume the last column is the target. (default: {None})
        """
        self.graph = graph
    ...
    def _get_features(self):
        not_features = [self.target, '']
        if self.target is None:
            return [self.graph[i].name for i in range(len(self.graph)-1) if len(self.graph[i].name) > 0]
        else:
    ...
    def _kernel_matrix(self):
        ...
        denom = np.multiply(np.multiply(binom(no_features, nonzero_feat), nonzero_feat), (no_features - nonzero_feat))
        nom = no_features - 1
