...
class HttpConnectionManager(betterproto.Message):
    ...
    # this property.
    route_config: _____config_route_v4_alpha__.RouteConfiguration = betterproto.message_field(
    ...
    common_http_protocol_options: _____config_core_v4_alpha__.HttpProtocolOptions = betterproto.message_field(
        35
    )
    # Additional HTTP/1 settings that are passed to the HTTP/1 codec.
    http_protocol_options: _____config_core_v4_alpha__.Http1ProtocolOptions = betterproto.message_field(
        8
    )
    # Additional HTTP/2 settings that are passed directly to the HTTP/2 codec.
    http2_protocol_options: _____config_core_v4_alpha__.Http2ProtocolOptions = betterproto.message_field(
        9
    )
    ...
    # server_name.
    server_header_transformation: "HttpConnectionManagerServerHeaderTransformation" = betterproto.enum_field(
        34
    )
    ...
    internal_address_config: "HttpConnectionManagerInternalAddressConfig" = betterproto.message_field(
        25
    )
    ...
    forward_client_cert_details: "HttpConnectionManagerForwardClientCertDetails" = betterproto.enum_field(
        16
    )
    ...
    # invalid_http_message for HTTP/1 and HTTP/2 are desired, one *must* use the
    # new HTTP/2 option :ref:`override_stream_error_on_invalid_http_message <envo
    # y_v3_api_field_config.core.v3.Http2ProtocolOptions.override_stream_error_on
    # _invalid_http_message>` *not* the deprecated but similarly named
...
class LocalReplyConfig(betterproto.Message):
    ...
    # body_format. .. code-block::   text_format:
    # %LOCAL_REPLY_BODY%:%RESPONSE_CODE%:path=$REQ(:path)% The following response
    # body in `plain/text` format will be generated for a request with local
    # reply body of "upstream connection error", response_code=503 and path=/foo.
    # .. code-block::   upstream connect error:503:path=/foo  Example two:
    # application/json body_format. .. code-block::  json_format:    status:
    # %RESPONSE_CODE%    message: %LOCAL_REPLY_BODY%    path: $REQ(:path)% The
    # following response body in "application/json" format would be generated for
    # a request with local reply body of "upstream connection error",
    # response_code=503 and path=/foo. .. code-block:: json  {    "status": 503,
    # "message": "upstream connection error",    "path": "/foo"  }
    body_format: _____config_core_v4_alpha__.SubstitutionFormatString = betterproto.message_field(
        2
    )
...
class ResponseMapper(betterproto.Message):
    ...
    filter: _____config_accesslog_v4_alpha__.AccessLogFilter = betterproto.message_field(
        1
    )
    ...
    body_format_override: _____config_core_v4_alpha__.SubstitutionFormatString = betterproto.message_field(
        4
    )
...
class Rds(betterproto.Message):
    ...
    config_source: _____config_core_v4_alpha__.ConfigSource = betterproto.message_field(
        1
    )
    ...
    rds_resource_locator: ______udpa_core_v1__.ResourceLocator = betterproto.message_field(
        3, group="name_specifier"
    )
...
class ScopedRoutes(betterproto.Message):
    ...
    rds_config_source: _____config_core_v4_alpha__.ConfigSource = betterproto.message_field(
        3
    )
    ...
    scoped_route_configurations_list: "ScopedRouteConfigurationsList" = betterproto.message_field(
        4, group="config_specifier"
    )
...
class ScopedRds(betterproto.Message):
    ...
    scoped_rds_config_source: _____config_core_v4_alpha__.ConfigSource = betterproto.message_field(
        1
    )
...
class HttpFilter(betterproto.Message):
    ...
    # HTTP listener responds with 500. Extension configs delivered through this
    # mechanism are not expected to require warming (see
    # https://github.com/envoyproxy/envoy/issues/12061).
    config_discovery: _____config_core_v4_alpha__.ExtensionConfigSource = betterproto.message_field(
        5, group="config_type"
    )
