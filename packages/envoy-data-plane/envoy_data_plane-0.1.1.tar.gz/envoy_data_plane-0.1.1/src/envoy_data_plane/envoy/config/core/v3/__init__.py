...
class GrpcServiceGoogleGrpcCallCredentials(betterproto.Message):
    ...
    google_compute_engine: betterproto_lib_google_protobuf.Empty = betterproto.message_field(
        2, group="credential_specifier"
    )
...
class HealthCheck(betterproto.Message):
    ...
    transport_socket_match_criteria: betterproto_lib_google_protobuf.Struct = betterproto.message_field(
        23
    )
...
class HealthCheckHttpHealthCheck(betterproto.Message):
    ...
    service_name_matcher: ___type_matcher_v3__.StringMatcher = betterproto.message_field(
        11
    )
...
class SubstitutionFormatString(betterproto.Message):
    ...
    # is described in :ref:`format string<config_access_log_format_strings>`. ..
    # code-block::   text_format:
    # %LOCAL_REPLY_BODY%:%RESPONSE_CODE%:path=$REQ(:path)% The following plain
    # text will be created: .. code-block::   upstream connect
    # error:204:path=/foo
    ...
    # for details. .. code-block::  json_format:    status: %RESPONSE_CODE%
    # message: %LOCAL_REPLY_BODY% The following JSON object would be created: ..
    # code-block:: json  {    "status": 500,    "message": "My error message"  }
