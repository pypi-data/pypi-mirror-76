...
class Cluster(betterproto.Message):
    """Configuration for a single upstream cluster. [#next-free-field: 51]"""
    ...
    load_assignment: __endpoint_v3__.ClusterLoadAssignment = betterproto.message_field(
        33
    )
    ...
    upstream_http_protocol_options: __core_v3__.UpstreamHttpProtocolOptions = betterproto.message_field(
        46
    )
    # Additional options when handling HTTP requests upstream. These options will
    # be applicable to both HTTP1 and HTTP2 requests.
    common_http_protocol_options: __core_v3__.HttpProtocolOptions = betterproto.message_field(
        29
    )
    # Additional options when handling HTTP1 requests.
    http_protocol_options: __core_v3__.Http1ProtocolOptions = betterproto.message_field(
        13
    )
    ...
    http2_protocol_options: __core_v3__.Http2ProtocolOptions = betterproto.message_field(
        14
    )
...
class ClusterEdsClusterConfig(betterproto.Message):
    ...
    # [#not-implemented-hide:]
    eds_resource_locator: ____udpa_core_v1__.ResourceLocator = betterproto.message_field(
        3
    )
...
class ClusterLbSubsetConfig(betterproto.Message):
    ...
    # Policy.NO_FALLBACK>`.
    default_subset: betterproto_lib_google_protobuf.Struct = betterproto.message_field(
        2
    )
...
class ClusterCommonLbConfig(betterproto.Message):
    ...
    # RingHashLb, etc.)
    consistent_hashing_lb_config: "ClusterCommonLbConfigConsistentHashingLbConfig" = betterproto.message_field(
        7
    )
...
class ClusterPrefetchPolicy(betterproto.Message):
    ...
    # Indicates how many many streams (rounded up) can be anticipated per-
    # upstream for each stream, useful for high-QPS or latency-sensitive
    # services. For example if this is 2, for an incoming HTTP/1.1 stream, 2
    # connections will be established, one for the new incoming stream, and one
    # for a presumed follow-up stream. For HTTP/2, only one connection would be
    # established by default as one connection can serve both the original and
    # presumed follow-up stream. In steady state for non-multiplexed connections
    # a value of 1.5 would mean if there were 100 active streams, there would be
    # 100 connections in use, and 50 connections prefetched. This might be a
    # useful value for something like short lived single-use connections, for
    # example proxying HTTP/1.1 if keep-alive were false and each stream resulted
    # in connection termination. It would likely be overkill for long lived
    # connections, such as TCP proxying SMTP or regular HTTP/1.1 with keep-alive.
    # For long lived traffic, a value of 1.05 would be more reasonable, where for
    # every 100 connections, 5 prefetched connections would be in the queue in
    # case of unexpected disconnects where the connection could not be reused. If
    # this value is not set, or set explicitly to one, Envoy will fetch as many
    ...
    prefetch_ratio: Optional[float] = betterproto.message_field(
