        ...
        if in_ext == '.ipynb' and out_ext == '.py':
            ...
            if path.exists(out_file):
                with open(out_file, 'r', encoding='utf-8') as f:
                    exist_notebook = json.load(f)
                    if exist_notebook == notebook:
                        print(f'{out_file} is already exist, not changed')
                        continue
...
def main():
    argv = sys.argv
    if len(argv) < 2:
        print('Usage: py2ipynb in.py')
        sys.exit(1)

    convert(in_files=glob(argv[1]))
