...
class QPDPandasEngine(QPDEngine):
    ...
    def binary_logical_op(self, col1: Column, col2: Column, op: str) -> Column:
        if op == "and":
            ...
            if isinstance(s, pd.Series):
                s[(col1.native == 0) | (col2.native == 0)] = 0
            elif (col1.native == 0) | (col2.native == 0):
                s = 0
        elif op == "or":
            s = col1.native + col2.native
            if isinstance(s, pd.Series):
                s[(col1.native > 0) | (col2.native > 0)] = 1
            elif (col1.native > 0) | (col2.native > 0):
                s = 1
        else:
            raise NotImplementedError(f"{op} is not supported")
    ...
    def show(self, df: DataFrame) -> DataFrame:
        print(self.to_native(df))
        return df

    def show_col(self, col: Column) -> Column:
        print(col.name, col.native)
        return col
    ...
    def is_in(self, col: Column, *values: Any, positive: bool) -> Column:
        ...
        if positive:
            ...
            for c in cols:
                o = o & col.native != c.native
        if isinstance(o, pd.Series) and isinstance(col.native, pd.Series):
            o[col.native.isnull()] = None
        return Column(o)
    ...
    def is_between(
        ...
        if positive:
            s: Any = (lower.native <= col.native) & (col.native <= upper.native)
        else:
            s = (lower.native > col.native) | (col.native > upper.native)
        if isinstance(col.native, pd.Series):
            s[col.native.isnull()] = None
        if isinstance(lower.native, pd.Series):
            s[lower.native.isnull()] = None
        if isinstance(upper.native, pd.Series):
            s[upper.native.isnull()] = None
        return Column(s)

    def filter_df(self, df: DataFrame, cond: Column) -> DataFrame:
        ndf = self.to_native(df)[cond.native > 0]
    ...
    def _set_op_result_to_none(self, series: Any, s1: Any, s2: Any) -> Column:
        ...
        if isinstance(s1, pd.Series):
            series[s1.isnull()] = None
        if isinstance(s2, pd.Series):
            series[s2.isnull()] = None
        return Column(series)
