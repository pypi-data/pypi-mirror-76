    ...
    class Tests(TestsBase):
        ...
        def test_comparison_op(self):
            def assert_eq(df, first, second, op, expected):
                ...
                res = self.op.comparison_op(first, second, op).native.tolist()
        ...
        def test_binary_logical_op(self):
            def assert_eq(df, first, second, op, expected):
                ...
                if isinstance(second, str):
                    ...
                    second = Column(second)
                res = self.op.binary_logical_op(first, second, op).native.tolist()
        ...
        def test_is_value(self):
            def assert_eq(values, expr, positive, expected):
                ...
                res = self.op.is_value(col, IsValueSpec(expr, positive)).native
        ...
        def test_is_in(self):
            def assert_eq(df, col, positive, collection, expected):
                ...
                res = self.op.is_in(df[col], *comp, positive=positive).native.tolist()
                assert_df_eq(pd.DataFrame({"a": res}), pd.DataFrame({"a": expected}))
        ...
        def test_case_when(self):
            def assert_eq(df, *cols, expected):
                ...
                res = self.op.case_when(*cols).native
                assert expected == res.tolist()
        ...
        def test_drop_duplicates(self):
            def assert_eq(df, expected, expected_cols):
                res = self.op.to_native(self.op.drop_duplicates(df))
        ...
        def test_union(self):
            def assert_eq(df1, df2, unique, expected, expected_cols):
                res = self.op.to_native(self.op.union(df1, df2, unique=unique))
                assert_df_eq(res, expected, expected_cols)
        ...
        def test_intersect(self):
            def assert_eq(df1, df2, unique, expected, expected_cols):
                res = self.op.to_native(self.op.intersect(df1, df2, unique=unique))
                assert_df_eq(res, expected, expected_cols)
        ...
        def test_except(self):
            def assert_eq(df1, df2, unique, expected, expected_cols):
                res = self.op.to_native(self.op.except_df(df1, df2, unique=unique))
                assert_df_eq(res, expected, expected_cols)
        ...
        def test_sort(self):
            ...
            df = self.to_native_df(
                [[0, 10, None], [1, None, 100], [None, 20, 300]], columns=list("abc")
            )
            assert_eq(df, [1, 2, 3])
        ...
        def test_agg(self):
            def assert_eq(pdf, gp_keys, gp_map, expected, expected_cols):
                op = self.op
                df = op.to_df(pdf)
                res = op.to_native(op.group_agg(df, gp_keys, gp_map))
                assert_df_eq(res, expected, expected_cols)

            nan = float("nan")
            pdf = self.to_native_df(
                [[0, 1], [0, 1], [1, None], [1, 5], [0, None]], columns=["a", "b"]
            )
            assert_eq(
                pdf,
                ["a"],
                ...
                [[1.0, 1.0, nan, 1.0], [nan, 5.0, 5.0, 5.0]],
                ["b1", "b2", "b3", "b4"],
            )
            pdf = self.to_native_df(
                [[0, 1], [0, 1], [0, 2], [1, 5], [0, None]], columns=["a", "b"]
            )
            assert_eq(
                pdf,
                ["a"],
            ...
            pdf = self.to_native_df(
                [[0, 1], [0, 1], [0, 2], [1, None], [0, None]], columns=["a", "b"]
            )
            assert_eq(
                pdf,
                ["a"],
            ...
            pdf = self.to_native_df(
                [[0, 1], [0, 1], [0, 4], [1, 5], [0, None]], columns=["a", "b"]
            )
            assert_eq(
                pdf,
                ["a"],
            ...
            pdf = self.to_native_df(
                [[0, 1], [0, 1], [0, 4], [1, None], [0, None]], columns=["a", "b"]
            )
            ...
            assert_eq(
                ...
                ["a"],
                {
                    "a1": ("a", AggFunctionSpec("first", unique=False, dropna=False)),
                    "a2": ("*", AggFunctionSpec("count", unique=True, dropna=False)),
                    "a3": ("c", AggFunctionSpec("first", unique=True, dropna=False)),
                ...
                [
                    [0.0, 2.0, nan, 3.0, 1.0, 2.0],
                    [1.0, 1.0, nan, 1.0, nan, 1.0],
                    [nan, 1.0, nan, 1.0, nan, 1.0],
                ],
                ["a1", "a2", "a3", "a4", "a5", "a6"],
            )

        def test_agg_no_group(self):
            def assert_eq(pdf, gp_map, expected, expected_cols):
                op = self.op
                df = op.to_df(pdf)
                res = op.to_native(op.group_agg(df, [], gp_map))
                assert_df_eq(res, expected, expected_cols)
            ...
            pdf = self.to_native_df(
                [[0, 1], [0, 1], [0, 4], [0, None]], columns=["a", "b"]
            )
            assert_eq(
                pdf,
                {
            ...
            pdf = self.to_native_df([[0, 1], [0, 1], [1, None]], columns=["a", "b"])
            # count on single column
            assert_eq(
                pdf,
                {
        ...
        def test_window_row_number(self):
            ...
            a = self.to_native_df([[0, 1], [0, 2]], columns=["a", "b"])
            self.w_eq(
            ...
            a = self.to_native_df(
                [[0, 1], [0, None], [None, 1], [None, None]], columns=["a", "b"]
            )
            self.w_eq(
                a,
            ...
            a = self.to_native_df([[0, 1], [0, 4], [2, 3]], columns=["a", "b"])
            self.w_eq(
                a,
                make_func([], [("b", False)]),
                [],
        ...
        def test_window_ranks(self):
            ...
            a = self.to_native_df(
                [[0, 1], [0, 1], [None, 1], [0, None]], columns=["a", "b"]
            )
            self.w_eq(
                a,
                make_func("rank", [], ["b"]),
        ...
        def test_window_sum(self):
            ...
            a = self.to_native_df(
                [[0, 1], [None, 1], [0, None], [0, 2]], columns=["a", "b"]
            )
            self.w_eq(
                a,
                make_func(["a"], []),
        ...
        def test_window_simple_agg(self):
            ...
            a = self.to_native_df(
                [[0, 1], [None, 1], [0, None], [0, 3]], columns=["a", "b"]
            )
            self.w_eq(
                a,
        ...
        def w_eq(self, pdf, func, cols, dest, expected, expected_cols):
            df = self.op.to_df(pdf)
            cols = [
                x if isinstance(x, ArgumentSpec) else ArgumentSpec(False, x)
                for x in cols
            ]
            res = self.op.to_native(self.op.window(df, func, cols, dest))
            assert_df_eq(res, expected, expected_cols)
