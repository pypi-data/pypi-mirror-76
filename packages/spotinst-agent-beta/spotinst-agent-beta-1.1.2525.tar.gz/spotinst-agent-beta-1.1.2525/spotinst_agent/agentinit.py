...
import logging
from utils import safe_read_simple_url, safe_get_json, retrieve_creds, get_gcp_metadata, get_azure_metadata
...
def getGcpInstanceMetadata(locator_url, path_params, log):
    ...
    if locator_url:
        if locator_url.startswith("http://"):
            response = get_gcp_metadata(locator_url, path_params, log)
            response = str(response)
...
def getAzureInstanceId(locator_url, log):
    ...
    if locator_url:
        if locator_url.startswith("http://"):
            response = get_azure_metadata(locator_url, log)
            response = str(response)
