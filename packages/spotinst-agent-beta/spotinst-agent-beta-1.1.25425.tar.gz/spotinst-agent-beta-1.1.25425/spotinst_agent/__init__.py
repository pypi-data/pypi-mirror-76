...
def install_collector(collector_name, modules_path, s3_base_collectors_link):
    ...
    try:
        ...
        print ("Failed to install Collector: %s. %s" % (e, traceback.format_exc()))


def get_worker_files(modules_path, s3_base_workers_link, worker_name):
    py_response = urlopen(s3_base_workers_link + '/' + worker_name + '/' + worker_name + '.py')
    yml_response = urlopen(s3_base_workers_link + '/' + worker_name + '/' + worker_name + '.yml')
...
def get_collector_files(collector_name, modules_path, s3_base_collectors_link):
    with open(modules_path + '/' + collector_name + '.py', 'wb') as collector:
        print(s3_base_collectors_link + '/' + collector_name + '/' + collector_name + '.py')
        response = urlopen(s3_base_collectors_link + '/' + collector_name + '/' + collector_name + '.py')
        collector.write(response.read())
    with open(modules_path + '/' + collector_name + '.yml', 'wb') as collector_cfg:
        response = urlopen(s3_base_collectors_link + '/' + collector_name + '/' + collector_name + '.yml')
        collector_cfg.write(response.read())


def handle_prerequisites(s3_base_module_link, module_name):
    try:
        response = urlopen(s3_base_module_link + '/' + module_name + '/' + module_name + '.req')
        ...
        print ("Failed to handle prerequisites: %s. %s" % (e, traceback.format_exc()))
        pass
