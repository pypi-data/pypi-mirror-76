...
def install_collector(collector_name, modules_path, s3_base_collectors_link):
    ...
    try:
        ...
        print ("Failed to load Module: %s. %s" % (e, traceback.format_exc()))


def get_worker_files(modules_path, s3_base_workers_link, worker_name):
    pyUrl = urllib.parse.quote(urllib.parse.urljoin(s3_base_workers_link, worker_name, "%s.py" % worker_name))
    ymlUrl = urllib.parse.quote(urllib.parse.urljoin(s3_base_workers_link, worker_name, "%s.yml" % worker_name)) 

    py_response = urllib.request.urlopen(pyUrl)
    yml_response = urllib.request.urlopen(ymlUrl)
...
def get_collector_files(collector_name, modules_path, s3_base_collectors_link):
    with open(modules_path + '/' + collector_name + '.py', 'wb') as collector:
        pyUrl = urllib.parse.quote(urllib.parse.urljoin(s3_base_collectors_link, collector_name, "%s.py" % collector_name))
        response = urllib.request.urlopen(pyUrl)
        collector.write(response.read())
    with open(modules_path + '/' + collector_name + '.yml', 'wb') as collector_cfg:
        ymlUrl = urllib.parse.quote(urllib.parse.urljoin(s3_base_collectors_link, collector_name, "%s.yml" % collector_name))
        response = urllib.request.urlopen(ymlUrl)
        collector_cfg.write(response.read())


def handle_prerequisites(s3_base_module_link, module_name):
    try:
        reqUrl = urllib.parse.quote(urllib.parse.urljoin(s3_base_module_link, module_name, "%s.req" % module_name))
