...
@LastEditTime: 2020-08-07 10:02:15
...
class LoginHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        BehaviorModel().report_behavior(app_id, act_id, open_id, owner_open_id, 'VisitCountEveryDay', 1)
        BehaviorModel().report_behavior(app_id, act_id, open_id, owner_open_id, 'VisitManCountEveryDay', 1)
        if user_info.is_new == 1:
            BehaviorModel().report_behavior(app_id, act_id, open_id, owner_open_id, 'VisitManCountEveryDayIncrease', 1)
...
class SyncPayOrderHandler(SevenBaseHandler):
    """
    ...
    def get_async(self):
        ...
        if self.get_param("source_app_id") == config.get_value("client_template_id"):
            order_data = json.loads(self.TestOrder())
        else:
            order_data = TopModel().get_taobao_order(open_id, access_token)
        ...
        if add_lottery_count == True:
            ...
            BehaviorModel().report_behavior(app_id, act_id, open_id, act_info.owner_open_id, 'PayMoneyCount', decimal.Decimal(total_pay_prize))
            BehaviorModel().report_behavior(app_id, act_id, open_id, act_info.owner_open_id, 'PayerCount', 1)
            BehaviorModel().report_behavior(app_id, act_id, open_id, act_info.owner_open_id, 'PayCount', total_pay_num)
    ...
    def get_sku_name(self, good_id, sku_id, access_token):
        num_iids = good_id
        ...
        return TopModel().get_sku_name(num_iids, sku_id, access_token)
