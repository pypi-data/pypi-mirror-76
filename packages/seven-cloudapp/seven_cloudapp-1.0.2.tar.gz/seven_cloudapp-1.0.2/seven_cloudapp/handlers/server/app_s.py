...
@LastEditTime: 2020-08-07 16:25:34
...
class AppUpdateHandler(SevenBaseHandler):
    """
    @description: 前端版本更新
    """
    @filter_check_params()
    def get_async(self):
        ...
        rep_dic, app_info = TopModel().app_update(app_info, app_id, client_template_id, base_info.client_ver, access_token)
...
class AppInfoHandler(SevenBaseHandler):
    """
    @description: 小程序信息
    @param {type} 
    @return: 
    ...
    def get_async(self):
        ...
        dead_date = TopModel().get_dead_date(source_app_id, store_user_nick, access_token)

        login_log = LoginLogModel().get_entity("open_id=%s", order_by="id desc", params=open_id)

        if app_info:
            access_token = TopModel().get_token(source_app_id, access_token)
            if access_token != "":
