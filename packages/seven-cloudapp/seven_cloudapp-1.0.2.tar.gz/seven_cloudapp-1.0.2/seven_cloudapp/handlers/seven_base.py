...
@LastEditTime: 2020-08-07 16:45:54
...
class SevenBaseHandler(BaseApiHandler):
    ...
    def get_taobao_param(self):
        ...
        user_nick = self.get_param("user_nick", "loveyouhk")
        ...
        test_open_id = config.get_value("test_open_id")
        if test_open_id:
            open_id = test_open_id

        test_user_nick = config.get_value("test_user_nick")
        if test_user_nick:
            user_nick = test_user_nick

        test_source_app_id = config.get_value("test_source_app_id")
        if test_source_app_id:
            source_app_id = test_source_app_id

        if source_app_id == config.get_value("client_template_id") or source_app_id == config.get_value("server_template_id"):
    ...
    def acquire_lock(self, lock_name, acquire_time=10, time_out=5):
        ...
        while time.time() < end:
            if self.redis_init().setnx(lock, identifier):
                ...
                return identifier
            elif not self.redis_init().ttl(lock):
                self.redis_init().expire(lock, time_out)
            time.sleep(0.001)
        return False
