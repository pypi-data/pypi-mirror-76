...
@LastEditTime: 2020-08-07 10:55:19
...
class ActCreateHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if not app_info:
            if ":" in user_nick:
                return self.reponse_json_error("Error", "对不起，子帐号无法实例化")
            ...
            app_info = TopModel().instantiate_app(user_nick, open_id, description, icon, act_name, template_id, template_version, 1, access_token, name_ending)
...
class ActHandler(SevenBaseHandler):
    ...
    def post_async(self):
        ...
        if act_id > 0:
            ...
            act_info.share_desc = share_desc
            act_info.rule_desc = rule_desc
...
class ActListHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if dict_app_id:
            ...
            page_list, total = ActInfoModel().get_page_list("*", page_index, page_size, condition, params=app_id)
            new_list = []
...
class ActReviewHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if act_info_total > 9:
            self.reponse_json_error("OverAct", "对不起，活动不可超过10个")
        else:
            act_info_model.update_table("is_del=0,is_release=1", "id=%s", act_id)
        self.reponse_json_success()
