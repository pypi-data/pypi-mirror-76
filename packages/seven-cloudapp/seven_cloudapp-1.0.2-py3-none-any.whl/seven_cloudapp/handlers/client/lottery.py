...
@LastEditTime: 2020-07-30 10:28:21
...
class LotteryHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        try:
            ...
            self.logger_info.info(str(ex) + "【拆盒】")
...
class TestTransHandler(SevenBaseHandler):
    """
    @description: 测试事务(作为事务例子)
    @param {type} 
    @return: 
    @last_editors: CaiYouBin
    """
    def get_async(self):
        UserInfoModel().del_entity("id=0")
        db_transaction = DbTransaction(db_config_dict=config.get_value("db_mang_he"))

        user_info_model = UserInfoModel(db_transaction=db_transaction)
        machine_info_model = MachineInfoModel(db_transaction=db_transaction)
        act_prize_model = ActPrizeModel(db_transaction=db_transaction)
        user_id = 0
        ...
        try:
            ...
            user_info = UserInfo()
            user_info.id = "1212121"
            user_info.act_id = 1
            user_info_model.add_entity(user_info)
            user_id = user_info_model.get_entity(order_by='id desc')

            machine_info = MachineInfo()
            machine_info.act_id = user_info.id
            machine_info_model.add_entity(machine_info)

            # act_prize_id = act_prize_model.update_table("ide=1", where="id=162")
