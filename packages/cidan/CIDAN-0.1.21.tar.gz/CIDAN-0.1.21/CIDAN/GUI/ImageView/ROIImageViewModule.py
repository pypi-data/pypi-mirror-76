...
class ROIImageViewModule(ImageViewModule):
    ...
    def set_background(self, name, func_name, update_image=True, reset_override=False):
        if (not self.resetting_view or reset_override):
            self.main_widget.console.updateText("Setting current background to: %s"%func_name)
            ...
            #         [-1, 1])
                ...
                try:
                    ...
                    self.main_widget.console.updateText("Can't display EigenNorm image, Eigen vectors aren't currently generated or valid")
    ...
    def updateImageDisplay(self, new=False):
        ...
        try:
            ...
            background_min= self.current_background.min()
            background_image_scaled = (self.current_foreground_intensity*(self.current_background-background_min) * 235 / (
                (background_max-background_min) if (background_max-background_min) != 0 else 1))
            ...
            if new and not hasattr(self.main_widget.data_handler,
                ...
                self.image_item.ui.histogram.setHistogramRange(0, 255)
            elif new:
                # if self.add_image:
    ...
    def selectRoi(self, num):
        try:
            ...
            self.main_widget.console.updateText("Please regenerate ROIs before trying this operation")
