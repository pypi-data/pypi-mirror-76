...
class DataHandler:
    ...
    def calculate_statistics(self):
        ...
        for num, roi in enumerate(self.rois):
            A[roi, num] = 1
        rval, significant_samples = classify_components_ep(self.dataset_trials_filtered_loaded, A, np.vstack([self.get_time_trace(x+1) for x in range(len(self.rois))]))
        pass
    ...
    def export(self):
        ...
        roi_image_outline = np.hstack(
            [self.edge_roi_image_flat,
             np.zeros(shape),
             np.zeros(shape)])
        max_image = np.reshape(np.max([self.max_images[x] for x in self._trials_loaded_indices],axis=0),(-1,1))
        ...
        roi_image_outline_w_background = roi_image_outline +np.hstack(
                [max_image, max_image,max_image])/max_image.max()*255
        self.save_image(roi_image_blob, os.path.join(self.save_dir_path, "roi_blob.png"))
