...
class ThingTalk(AsyncMixin):
    ...
    async def config_on_startups(self):
        on_startups = [init, self.start, self.zmq_sub, ]
...
async def inner():
    ...
    socket.setsockopt(zmq.SUBSCRIBE, b'topic\0')
    try:
        while True:
            print("begin recv json from zmq")
            print(await socket.recv_json())
    except asyncio.CancelledError:
        print("cancel zmq sub")
