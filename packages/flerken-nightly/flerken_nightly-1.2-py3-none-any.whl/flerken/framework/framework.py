...
class Trainer(DinamicProperty):
    ...
    def run_epoch(self, dataloader, state, metrics=[], checkpoint=lambda: None, allocate_input=True,
                  allocate_gt=True, send={}):
        self.state = state
        j = -1
        iterations = len(dataloader)
        if torch.is_grad_enabled() and 'loss' not in metrics:
            metrics.append('loss')
        self.set_model_training(torch.is_grad_enabled())
        self.iterating = True
