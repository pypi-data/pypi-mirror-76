    ...
    async def _run_action(
        ...
        try:
            ...
            logger.error(
                ...
                "more information. Retrying...".format(tracker.sender_id, action.name())
            )
            logger.error(e, exc_info=True)
            # Temporary fix for the CancelledError in aiohttp when users disconnect and try to call remote actions
            events = await self._retry_run_action(action, tracker, output_channel, nlg, policy, confidence)
            if events is False:
                return events
