...
def create_app(
    ...
    @app.get("/profiler/print")
    async def print_stats(request: Request):
        # app.pr.disable()
        s = io.StringIO()
        ps = pstats.Stats(app.pr, stream=s).sort_stats('cumulative')
        ps.print_stats(40)
        ps.dump_stats("/tmp/result.profile")
        logger.info(s.getvalue())
        return response.text(s.getvalue())

    @app.get("/profiler/download")
    async def download_profile_data(request: Request):
        return await response.file("/tmp/result.profile", filename="result.profile")

    @app.get("/profiler/start")
    async def start_profling(request: Request):
        app.pr.enable()
