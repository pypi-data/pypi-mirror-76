...
class CallbackView(BaseView, metaclass=MetaCallbackView):
    ...
    @classmethod
    async def validate(cls, call: types.CallbackQuery, call_data: CallbackData):
        pass

    @classmethod
    @abstractmethod
    async def main(cls, call: types.CallbackQuery, validated_data: dict, call_data: CallbackData):
        pass

    @classmethod
    async def generate_callback(cls, data: dict):
        return await CallbackData.create(cls, **data)
