...
class LazyResponse:
    ...
    def _truncate_long_strings(self):
        url = truncate_if_necessary(self.url, REPR_LIMIT)
        ...
        return url, method, request_data, request_headers

    def _load_response(self):
        if not self._response_loaded:
            url, method, request_data, request_headers = self._truncate_long_strings()
            logger.info(f"REQUEST: {url} | method: {method} | data: {request_data} | headers: {request_headers}")
            start = time.time()
            self._response = self._get_response()
            self.response_time = time.time() - start
            self._response_loaded = True
            logger.info(f"RESPONSE: {url} | {self.status} | {self.response_time:.4f} seconds")
    ...
    def __repr__(self):
        url, method, request_data, _ = self._truncate_long_strings()
        return f"LazyResponse(request_data={request_data}, url={url}, method={method})"
