...
class Learner():
    ...
    def __init__(self, data_dir, output_dir, lr = 5e-5, train_bs = 1, eval_bs = 1, model_type = 'bert', 
                model_name = 'bert-base-multilingual-cased', save_steps = 1, max_seq_length = 256, seed = 42,
                weight_decay = 0.0, adam_epsilon = 1e-8, max_grad_norm = 1.0, num_train_epochs = 5):
    ...
    def get_labels(self):
        all_path = self.data_dir / 'all.txt'
        labels = []
    ...
    def train(self, train_dataset, valid_dataset, model, tokenizer, labels):
        ...
        for _ in train_iterator:
            ...
            results = self.evaluate(self, model, tokenizer, labels, 'validation')
    ...
    def evaluate(self, model, tokenizer, labels, mode, prefix=''):
        eval_dataset = self.load_and_cache_examples(tokenizer, labels, mode=mode)
    ...
    def fit(self):
        ...
        global_step, tr_loss = self.train(
            train_dataset, valid_dataset, self.model, self.tokenizer, self.labels)
        logger.info(' global_step = %s, average loss = %s', global_step, tr_loss)

        # Evaluation
        results = {}

        results = self.evaluate(self.model, self.tokenizer, self.labels, mode='validation')
        print(tabulate(results.items(),headers=['metric','score']))
        preds = self.evaluate(self.model, self.tokenizer, self.labels, mode='test')
