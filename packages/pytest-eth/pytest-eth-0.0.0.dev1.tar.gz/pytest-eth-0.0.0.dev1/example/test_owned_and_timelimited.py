...
#
#
# # Just use the 'eth' fixture in your pytest_ethereum-enabled tests
# def test_owned_and_timelimited(eth):
#
...
#     owned = eth.new(artifact_owned)
#     # Deploying Owned
#     owned = owned.deploy()
#
#     # You can specify a specific contract from the file
#     timelimited_factory = eth.new(artifact_timelimited)
...
#     assert owned.owner() == eth.accounts[0]  # Doesn't mine a block!
#
#     # non-'constant' functions transact by default
#     # NOTE: Transactions auto-mine (see eth-eth)
#     owned.changeOwner(eth.accounts[1])  # Transaction auto-mined into block
#     assert owned.owner() == eth.accounts[1]  # No transaction here
#
#     # Use this for asserting when a failed transaction should occur
#     starting_balance = eth.accounts[0].balance
#     with eth.tx_fails:
#         owned.changeOwner(eth.accounts[0])  # account 0 is no longer the owner!
#     # We can do multiple failures in here...
#     with eth.tx_fails:
#         owned.changeOwner(eth.accounts[2])  # account 2 isn't either
#
#     # No transactions were committed for these failures
#     assert starting_balance == eth.accounts[0].balance
#
#     # You can supply optional transaction params
#     owned.changeOwner(eth.accounts[0],
#                       transact={
#                           'from': eth.accounts[1],  # from a different sender
#                           # 'value': 100,  # send 100 wei in this transaction
#                           # You can also do other things... see web3.py for more info!
#                       }
#                       )
#     assert owned.owner() == eth.accounts[0]  # account[0] is the owner again!
#
#     # You can mine an empty block if you want
#     while timelimited.alive():  # This makes a call, so no transaction occurs
#         eth.mine_blocks()  # mines an empty block
#
#     # You can check the current timestamp
#     assert eth.now() >= timelimited.creationTime() + timelimited.duration()
...
#     print("Account 0 has", eth.accounts[0].balance, "Wei")
#     print("Account 1 has", eth.accounts[1].balance, "Wei")
#     print("Contract 'timelimited' has", timelimited.balance, "Wei")
#
#     # Send any address Ether
#     print("Account 2 has", eth.accounts[2].balance, "Wei")
#     eth.accounts[1].transfer(eth.accounts[2], 100)  # send 100 wei to address 2
#     print("Account 2 now has", eth.accounts[2].balance, "Wei")
