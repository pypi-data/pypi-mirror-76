...
def recompute_all(public_key = None, private_key= None, start_time= None, end_time= None, save_to_dropbox = False, dropbox_token = None, local_root_directory = './'):
    ...
    print(f'loading {trade_pkl_file_name}')
    trades = dbx.download_pkl(trade_pkl_file_name)
    ohlc_eth_binance = dbx.download_pkl(ohlc_eth_binance_pkl_file_name)

    ohlc_btc_binance = dbx.download_pkl(ohlc_btc_binance_pkl_file_name)
    ...
    ohlc_eth_cc = dbx.download_pkl(ohlc_eth_cc_pkl_file_name)
...
# ohlc_eth_binance
    ...
    initial_pnl = wh.at[wh.index[0], 'realized_pnl']
    starting_value = 10000
    wh.at[wh.index[0], 'realized_pnl'] = initial_pnl + starting_value  # qqlq k en mars
    ...
    x = td[['traded_eth', 'avg_eth_price', 'amount_traded_usd_eth', 'sens_trade_eth', 'trading_fees_eth_usd',
            'funding_fees_eth_usd', 'open_eth_cc', 'open_eth_bnc',
            'traded_xbt', 'avg_btc_price', 'amount_traded_usd_btc', 'sens_trade_xbt', 'trading_fees_xbt_usd',
            'funding_fees_xbt_usd', 'open_btc_cc', 'open_btc_bnc',
            'valuation',
            ...
            'trading_fees_eth_usd', 'funding_fees_eth_usd', 'slippage_eth_price_bnc', 'slippage_eth_cash_bnc',
            'trading_fees_xbt_usd', 'funding_fees_xbt_usd', 'slippage_btc_price_bnc', 'slippage_btc_cash_bnc']]
    ...
    x.columns = ['quantity_traded_eth', 'avg_traded_price_eth', 'amount_traded_usd_eth', 'n_trades_eth',
                 ...
                 'trading_fees_usd_eth', 'funding_fees_usd_eth', 'slippage_eth_price_bnc', 'slippage_eth_cash_bnc',
                 'trading_fees_usd_btc', 'funding_fees_usd_btc', 'slippage_btc_price_bnc', 'slippage_btc_cash_bnc']
...
def fetch_all_necessary_data(public_key = None, private_key= None, start_time= None, end_time= None, save_to_dropbox = False, dropbox_token = None, local_root_directory = './'):
    ...
    wh['dt_floor_h'] = wh.index.map(lambda x: x.floor('H'))
    wh = wh.groupby('dt_floor_h').sum()
    wh.at[wh.index[0], 'realizedPnl'] = 5000  # qqlq k en mars
    wh[
        'net_profit'] = wh.realizedPnl + wh.funding_fees_eth_usd + wh.funding_fees_xbt_usd + wh.trading_fees_eth_usd + wh.trading_fees_xbt_usd
    wh['valuation'] = wh.net_profit + wh.transfers_usd
    wh['valuation'] = wh.valuation.cumsum()
    td = td.join(wh.iloc[:, 4:])
    td['amount_traded_usd_eth'] = td.traded_eth * td.avg_eth_price
