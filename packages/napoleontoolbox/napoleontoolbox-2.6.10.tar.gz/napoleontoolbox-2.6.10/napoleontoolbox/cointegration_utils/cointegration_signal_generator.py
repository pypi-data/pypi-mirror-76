...
def pair_trading_shifted_correlation(pair_df = None, correlation_range = 100, suffixes=('_btc', '_eth'), **kwargs):
...
def pair_trading_shifted_correlation_with_cointegration(pair_df = None, correlation_range = 100, risk = 10, alpha = 0.05, suffixes=('_btc', '_eth'), **kwargs):
    len_histo = len(pair_df)
    assert len_histo > correlation_range
    log_returns_df = cointegration_utilities.add_log_returns(pair_df, suffixes=suffixes)
