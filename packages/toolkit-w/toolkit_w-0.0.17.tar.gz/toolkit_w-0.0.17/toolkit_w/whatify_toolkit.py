...
class MyWhatify:
    ...
    @classmethod
    def read_to_pandas_df(cls, user_token: str = None):
        df = my_toolkit.Connector.read_to_pandas_df(user_token=user_token)
        print('dele')
        return df

    @classmethod
    def write_pandas_df_to_s3(cls, df, user_token: str = None):  # s3_client, bucket, path, df):
        my_toolkit.Connector.write_pandas_df_to_s3(df=df, user_token=user_token)
