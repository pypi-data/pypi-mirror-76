...
class MyWhatify:
    ...
    def ls_client(cls, user_token: str = None):
        return my_toolkit.UserManagement.get_client_path(user_token=user_token)

    @classmethod
    def get_s3_boto_client(cls, user_token: str = None):
        credentials = my_toolkit.UserManagement.get_credentials(user_token=user_token)
        s3_client = boto3.client('s3',
                                 aws_access_key_id=credentials['access_key'],
                                 aws_secret_access_key=credentials['secret_key'],
                                 aws_session_token=credentials['session_token'])
        return s3_client, credentials['bucket'], credentials['path']

    @classmethod
    def read_to_pandas_df(cls, s3_client, bucket, path):
        obj = s3_client.get_object(Bucket=bucket, Key=path)
        df = pd.read_csv(obj['Body'])
        return df

    @classmethod
    def write_pandas_df_to_s3(cls, s3_client, bucket, path, df):
        csv_buffer = StringIO()
        df.to_csv(csv_buffer)
        s3_client.upload_fileobj(csv_buffer, bucket, path)
    ...
    def get_credentials(cls, api_key: str = None):
        retval = my_toolkit.UserManagement.get_credentials(user_token=api_key)
        return retval
