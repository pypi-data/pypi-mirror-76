...
class MyWhatify:
    ...
    @classmethod
    def login(cls, email, password, user_id: str = None):
    ...
    @classmethod
    def impersonate(cls, user_id, email: str = None, admin_token: str = None):
        with allure.step(' '.join(['impersonate to user_ID:', str(user_id)])):
    ...
    #############################
    @classmethod
    def get_api_post(cls, url, api_key=None):
    ...
    # def get_task_result_Tom(cls):
    #     #todo delete fun
    #     cls.login(email=vars_global.Users.AUTO_QA007.get_email(),
    #                password=vars_global.Users.AUTO_QA007.get_password())
    #
    #     task_id = 138637
    #     task_id_older = 138630
    #     jwt = cls.user_token.get_token()
    #     cls.halClient = HalClient(host=conf_server['HAL_HOST'], port=conf_server['HAL_PORT'], use_https=True)
    #     cls.report_client = ReportsClient(host=conf_server['REPORT_HOST'], port=conf_server['REPORT_PORT'])
    ...
    @classmethod
    def ls_client(cls, user_token: str = None):
        return my_toolkit.Connector.get_client_path(user_token=user_token)

    @classmethod
    def get_credentials(cls, api_key: str = None):
        return my_toolkit.Connector.get_credentials(user_token=api_key)
    ...
    @classmethod
    def read_to_pandas_df(cls, file_name: str, user_token: str = None):
        return my_toolkit.Connector.read_to_pandas_df(file_name=file_name, user_token=user_token)

    @classmethod
    def write_pandas_df_to_s3(cls, df, file_name: str, user_token: str = None):  # s3_client, bucket, path, df):
        return my_toolkit.Connector.write_pandas_df_to_s3(df=df, file_name=file_name, user_token=user_token)
