...
class ReqStockData():
    ...
    def _store_ETF_launch_date(self):
        ...
        for doc in docs:
            ...
            launch_date_dict = db_for_quant["hist_allstock"].find_one({"stock_code":stock_code})
            if launch_date_dict:
                launch_date = launch_date_dict.get("market_date")
                update_dic = {"stock_code":stock_code, "上市日期":launch_date}
                db_for_quant["newest_allstock"].update_one({"stock_code":stock_code}, {"$set":update_dic}, upsert=True)
    ...
    def store_hist_allstock(self, period=0):
        ...
        """
        # 从DC爬取所有stock的历史数据
        df = read_mongo(client["quant"]["newest_allstock"], {}, {"stock_code":1, "Zqdm":1})
        docs = df_to_docs(df)
        today_date = get_today_date(format_="%Y%m%d") # "20200803"
        previous_30_date = get_previous_date(today_date, days_str="30 d", format_="%Y%m%d") # "20200704"
        ...
        for doc in docs:
            ...
            df = self.req_hist_data(stock_code=stock_code, period=period, pt=False, beg=previous_30_date, end=today_date)
            ...
            for doc in docs:
                ...
                db_for_quant["hist_allstock"].update_one(
    ...
    def store_hist_index(self, period=0):
        ...
        # 从DC爬取所有index的历史数据
        stock_code_lst = ['1.000001', '1.000300', '0.399001', '0.399006', '0.399005']
        today_date = get_today_date(format_="%Y%m%d") # "20200803"
        previous_30_date = get_previous_date(today_date, days_str="30 d", format_="%Y%m%d") # "20200704"
        ...
        for stock_code in stock_code_lst:
            ...
            for doc in docs:
                market_date = doc.get("market_date")
                db_for_quant["hist_index"].update_one(
    ...
    def store_trend_zxstock(self, period=0):
        """
        ...
        # 从DC爬取'zx部分'的 分时数据
        # stock_code_lst = get_zxstock_lst() # # TODO: 获取zx的stock
        stock_code_lst = [
            "1.000001", "0.002505", "0.002168", "1.600757", "0.159995", "1.600977",
            "1.600855", "1.603042", "1.601398", "0.002230", "1.600016", "0.002415",
            "1.600685", "0.002352", "0.000651", "0.002594", "0.002959", "1.600685", "0.002594",
        ]
        ...
        for stock_code in stock_code_lst:
            ...
            for doc in docs:
                ...
                db_for_quant["trend_zxstock"].update_one(
    ...
    def store_newest_allstock(self):
        ...
        for doc in docs:
            Zqdm = doc.get("Zqdm")
            # 一条条更新
            db_for_quant["newest_allstock"].update_one(
                {"Zqdm":Zqdm},
                {"$set":doc},
                upsert=True,
            )
    ...
    def get_ALL_df(self):
        "获取所有A股的最新相关数据"
        df = read_mongo(db_for_quant["newest_allstock"])
        return df

    def get_A_df(self):
        "获取所有A股的最新相关数据"
        df = read_mongo(db_for_quant["newest_allstock"], {"f1":3})
        return df
    ...
    def merge_Zqmc(self, df):
        ...
        """
        all_df = read_mongo(db_for_quant["newest_allstock"], {}, {"Zqdm":1, "stock_code":1, "Zqmc":1})
...
Q = ReqStockData() # 使用Q作为实例名 (历史原因导致)
...
class FirmOfferAccount(QuantUser):
    ...
    def __init__(self, user_name="LZC", cookies_dict=None, userId="32040006118200", original_password="0061883000", OriginalTotalCapitalValue=100000):
        """
            params:
                userId: zjzh
        ...
        # 当没有传入cookies_dict时, 重新模拟登录, 重新获取cookies_dict
        if cookies_dict == None:
            ...
            # 4. 获取validatekey_value (买卖中必要的参数验证)
            time.sleep(1)
            self._req_validatekey_value()
        else:
            self.cookies_dict = cookies_dict

        # 5. 获取zz信息 (每次买卖执行前, 或者其他重要操作前, 都需要执行一遍该函数: 用于更新账户资金数据)
    ...
    def req_my_stock(self, stock_code_lst=[]):
        ...
        chicang_stock_df = Q.req_my_stock(stock_code_lst=stock_code_lst)
    ...
    def store_deal_sp(self):
        ...
        for doc in docs:
            ...
            # 2. 将 '所有历史成交数据' 一条条的插入
            db_for_quant["deal_sp"].update_one(
    ...
    def store_asset_sp(self, forced_update=False):
        ...
        for doc in docs:
            ...
            db_for_quant["asset_sp"].update_one(
...
def overview_asset(user_name="LZC", need_to_convert_date=True):
    """
        function:
    ...
    df = read_mongo(db_for_quant["asset_sp"], need_to_convert_date=need_to_convert_date)
...
def overview_deal(user_name="LZC", Zqmc=False):
    """
        function: 观测历史deal数据
    ...
    # 1. 获取mongo中的asset数据, 并透视
    df = read_mongo(db_for_quant["deal_sp"])
    df = df.query(f"user_name == '{user_name}'")
    ...
    stock_code_lst = Q._Zqdm_to_StockCode(Zqdm_lst=Zqdm_lst)
    newest_df = Q.req_newest_data(secids_lst=stock_code_lst) # 这里获取'最新价格'需要依赖: newest_allstock中是否记录该Zqdm对应的 stock_code
    ...
    if Zqmc == True:
        final_df = Q.merge_Zqmc(final_df)
...
def main():
    ...
    # get_live_data()
    # A = FirmOfferAccount() # 生成FOA对象可以大致能使用一天, 后续只要调用具体方法即可!!
    # df = A.req_deal_data()
    # print(df)
