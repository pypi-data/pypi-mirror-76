...
def get_application(context, application):
    ...
    try:
        ...
        assert context.sandbox.default_application is not None, "\n".join((
            f"Default application was not found. Check your environment file!"
            f"This is indication that no application was defined in environment.py file."
        ))

    assert app_class_to_return is not None, "\n".join((
        f"Application '{application}' was not found.",
        f"Possibly wrongly defined application in environment.py file or incorrect use of decorator in .feature file.",
    ))
