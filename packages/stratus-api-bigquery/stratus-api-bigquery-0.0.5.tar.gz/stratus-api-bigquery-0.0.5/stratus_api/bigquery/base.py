...
def extract_attributes(body: dict) -> tuple:
    ...
    service_name = body.get('subscription', '').split('/')[-1].split('-')[1]
...
def process_promise_update_request(body):
    ...
    sig = update_promise_task.delay(status=status, service_name=service_name, external_id=external_id, results=results)
