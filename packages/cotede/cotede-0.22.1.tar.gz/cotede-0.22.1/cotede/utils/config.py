...
def fix_procedure(cfg):
    ...
    same procedure.
    """
    assert "variables" in cfg, "There is no variables in cfg"

    for v in cfg["variables"]:
        for c in cfg["variables"][v]:
            procedure = guess_procedure(c, cfg["variables"][v][c])
            if cfg["variables"][v][c] is None:
                cfg["variables"][v][c] = {"procedure": procedure}
            elif "procedure" not in cfg["variables"][v][c]:
                cfg["variables"][v][c]["procedure"] = procedure
