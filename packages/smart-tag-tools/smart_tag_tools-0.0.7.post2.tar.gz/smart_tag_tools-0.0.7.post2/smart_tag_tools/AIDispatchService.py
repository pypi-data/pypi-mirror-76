...
class AIDispatchService:
    ...
    def add_task(self, task_type, access_token, version, files, params={}, user_data={},
        ...
        try:
            res = requests.post(self.kwargs['url'], json=data, timeout=self.kwargs.get('timeout', 5)).json()
            if res['code'] == 0:
                return res['data']
            else:
    ...
    def get_task(self, callback_url: str):
        try:
            res = requests.get(callback_url, timeout=self.kwargs.get('timeout', 5)).json()
            if res.get('code') == 0:
