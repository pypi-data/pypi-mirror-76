...
class SmartTagsConvertHelper:
    ...
    def __kb_heartbeat_detection__(self):
        ...
        """
        try:
            res = requests.get(url='http://{}:{}{}/health-check'.format(self.kwargs['KbService']['ip'],
                                                                        self.kwargs['KbService']['port'],
                                                                        self.kwargs['KbService']['UrlPrefix']),
                               timeout=self.kwargs['KbService']['timeout'])
    ...
    def __kb_interface_call(self, route, data):
        ...
        """
        try:
            res = requests.post(url='http://{}:{}{}{}'.format(self.kwargs['KbService']['ip'],
                                                              self.kwargs['KbService']['port'],
                                                              self.kwargs['KbService']['UrlPrefix'], route),
                                json=data,
                                timeout=self.kwargs['KbService']['timeout']).json()
            if res['success']:
                return res['data']
        except:
            return None
