...
class SmartTagsConvertHelper:
    ...
    def __kb_heartbeat_detection__(self):
        ...
        """
        try:
            res = requests.get(url='http://{}:{}{}/health-check'.format(self.kwargs['kb_service']['ip'],
                                                                        self.kwargs['kb_service']['port'],
                                                                        self.kwargs['kb_service']['url_prefix']),
                               timeout=self.kwargs['kb_service']['timeout'])
    ...
    def __kb_interface_call(self, route, data):
        ...
        """
        try:
            res = requests.post(url='http://{}:{}{}{}'.format(self.kwargs['kb_service']['ip'],
                                                              self.kwargs['kb_service']['port'],
                                                              self.kwargs['kb_service']['url_prefix'], route),
                                json=data,
                                timeout=self.kwargs['kb_service']['timeout']).json()
            if res['success']:
                return res['data']
        except:
            return None
