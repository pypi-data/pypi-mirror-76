...
from ..utils.data.AssetTypes import assettypes
...
class User:
    ...
    def games(self):
        ...
        This will return all games created by the user including private ones, as of right now there is no option to filter through them.  
        You're gonna have to do that yourself.  

        Returns: List[Game object]
        """
        r = self.requests.get(f"https://www.roblox.com/users/inventory/list-json?assetTypeId=9&cursor=&itemsPerPage=100&pageNumber=1&placeTab=Created&userId={str(self.id)}")
        gameslist = []
        for game in r.json()["Data"]["Items"]:
            gameslist.append(Game(self.requests, game["Item"]["AssetId"]))
        return gameslist