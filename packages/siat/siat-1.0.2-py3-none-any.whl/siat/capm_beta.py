...
def capm_beta(stkcd,mktidx,start,end):
    ...
    if R is None:
        print("Error #1(capm_beta): failed to prepare capm data")
    ...
    #显示回归结果
    print(" \n***CAPM beta: Stock_return=alpha+beta*Market_return")
...
def prepare_capm_portfolio(tickerlist,sharelist,mktidx,start,end):
    ...
    if stock is None:
        print("Error #1(prepare_capm_portfolio): failed to get stock prices")
        print("Information:",tickerlist,start,end)              
        return None    
    market=get_price(mktidx,start,end)
    if market is None:
        print("Error #2(prepare_capm_portfolio): failed to get market index")
        print("Information:",mktidx,start,end)              
        return None
...
def capm_beta_portfolio_yearly(tickerlist,sharelist,mktidx,yearlist):
    ...
    R=prepare_capm_portfolio(tickerlist,sharelist,mktidx,start,end)
    if R is None:
        print("Error #1(capm_beta_portfolioyearly): failed to prepare capm data")
        print("Information:",tickerlist,sharelist,mktidx,start,end)              
        return      
