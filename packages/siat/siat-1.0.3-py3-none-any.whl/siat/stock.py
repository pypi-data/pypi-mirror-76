...
#==============================================================================
def comp_1security_2measures(df,measure1,measure2):
    """
    功能：对比绘制一只证券两个指标的折线图。
    输入：证券指标数据集df；行情类别measure1/2。
    输出：绘制证券行情双折线图，自动判断使用单轴或双轴坐标
    返回：无
    """
    #筛选证券指标，检验是否支持指标
    dfcols=list(df)
    ...
    plot_line2(df,ticker,measure1,label1,df,ticker,measure2,label2, \
                   ylabeltxt,titletxt,footnote,zeroline=zeroline)
...
#==============================================================================
def comp_2securities_1measure(df1,df2,measure):
    """
    功能：对比绘制两只证券的相同指标折线图。
    输入：指标数据集df1/2；证券代码ticker1/2；指标类别measure。
    输出：绘制证券指标双折线图，自动判断使用单轴或双轴坐标。
    返回：无
    """
    #筛选证券指标，检验是否支持指标
    dfcols=list(df1)
...
def compare_stock(tickers,measures,fromdate,todate):
    ...
    if (security_num == 1) and (measure_num >= 2):
        ...
        #绘制单个证券的双指标对比图
        comp_1security_2measures(pltdf1,measure1,measure2)
        ...
        #绘制双证券单指标对比图
        comp_2securities_1measure(pltdf1,pltdf2,measure1)
    else:
        print("*** 错误3(compare)，未能理解比较内容！")
        return
