...
def main():
    ...
    parser.add_argument('--mean', action='store_true',
                        help='use the mean average smiliness of a frame, not the total')
    ...
    with pkg_resources.path('choirless_smiler', 'smile_detector.tflite') as model_path:
        ...
        smile_score, image = smiler.find_smiliest_frame(ffg, mean=args.mean)
...
class Smiler():
    ...
    def find_smiliest_frame(self, frames, callback=None, mean=False):
        ...
        def detect(gray, frame):
            ...
            for rect in faces:
                try:
                    ...
                    smile_scores.append(pred)
                except Exception as e:
                    pass

            if smile_scores:
                if mean:
                    smile_score = np.mean(smile_scores)
                else:
                    smile_score = np.sum(smile_scores)
            else:
                smile_score = 0
