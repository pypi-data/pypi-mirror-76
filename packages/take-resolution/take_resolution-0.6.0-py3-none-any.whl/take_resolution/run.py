...
def run(sql_context: CONTEXT, bot_identity: str, min_date: str, max_date: str) -> tp.Dict[str, tp.Any]:
    ...
    date_column = params['bot_events_date_column']
    sp_df_events_date = filter_range_rows(sp_df_events, date_column, min_date, max_date)
    
    bot_id_column = params["bot_events_identity_column"]
    sp_df_events_filtered = filter_rows(sp_df_events_date, (bot_id_column, bot_identity))
    
    event_columns = params["bot_events_col_list"]
    sp_df_events_selected = select_columns(sp_df_events_filtered, event_columns)
