...
class SafetyCulture:
    def __init__(self, api_token):
        ...
        # self.rate = self.manager.Value('i', 0)
    ...
    def requests_exceptions(
        self, url, request_to_make, headers, wait_time=1.5, data=None
    ):
        try:
            ...
            if "x-rate-limit-remaining" in response_headers:
                if int(response_headers["x-rate-limit-remaining"]) < 50:
                    print("About to hit the rate limit, pausing for a minute")
                    time.sleep(60)
                    self.requests_exceptions(
                        url, request_to_make, wait_time=wait_time, data=data
                    )
            return response
    ...
    def authenticated_request_get(self, url):
        ...
        response = self.requests_exceptions(url, request_to_make, headers)
        return response
    ...
    def authenticated_request_delete(self, url):
        ...
        return self.requests_exceptions(url, request_to_make, self.custom_http_headers)
