...
def create_schema_documentation(xml_schema_string , json_schema_string, output_folder, output_name, glossary_string=None):
    ...
    if output_folder!=None:
        ...
        for root, dirs, files in os.walk(path):
            for file in files:
                path_after_doc = 'documentation' + root.split('documentation')[1]
                zipFile.write(os.path.join(root, file), os.path.join(output_name, path_after_doc, file))
    
        text = "var schemaString = vkbeautify.xml(`" + xml_schema_string + "`)"
        zipFile.writestr(os.path.join(output_name,'documentation','js','LIXI-Master-Schema.js'), text)
        
        if glossary_string !=None:
            text = "var glossaryString = vkbeautify.xml(`" + glossary_string + "`)"
            zipFile.writestr(os.path.join(output_name,'documentation','js','LIXI_Glossary.js'), text)        
    
        if json_schema_string:
            zipFile.writestr(os.path.join(output_name, output_name + '.json'), json_schema_string)
        else:
            zipFile.writestr(os.path.join(output_name, output_name + '.xsd'), xml_schema_string)
