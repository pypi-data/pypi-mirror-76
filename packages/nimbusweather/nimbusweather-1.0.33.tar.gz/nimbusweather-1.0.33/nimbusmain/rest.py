...
class NimbusThread(RESTThread):
    ...
    def __init__(self, device_id, location_id, structures, token, protocol_name="Nimbus", post_interval=None):
        ...
        self.url_template = "https://api-edge.nimbusweather.org/v3/data/location/%s/structure/%s/measurements/batch"
    ...
    def process_records(self, records):
        ...
        for key in self.structures:
            for count in range(self.max_tries):
                try:
                    params = self.get_params(records, self.structures[key]['params'])
                    url = self.url_template % (self.location_id, self.structures[key]['id'])
