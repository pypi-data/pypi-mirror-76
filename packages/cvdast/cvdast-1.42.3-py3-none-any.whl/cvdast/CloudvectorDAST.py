    ...
    def _process_event_data(self, apis_to_check=[]):
        ...
        for event in events:
            ...
            if str(event["attributes"]["http_path"]).lower() not in self.params_captured_in_traffic:
                self.params_captured_in_traffic[str(event["attributes"]["http_path"])] = {}

            # print("\n\n\n\n\n\n\n")
            for param, value in req_params_found.items():
                # if "severity" in param:
                #     print(str(event) + "\n\n")
                if param in self.params_captured_in_traffic[str(event["attributes"]["http_path"])]:
                    # self.params_captured_in_traffic[str(event["attributes"]["http_path"]).lower()] = {}
                    if value not in self.params_captured_in_traffic[str(event["attributes"]["http_path"])][
                        param]:
                        self.params_captured_in_traffic[str(event["attributes"]["http_path"])][param].append(
                            value)
                else:
                    self.params_captured_in_traffic[str(event["attributes"]["http_path"])][param] = [value]
                try:
                    self.params_captured_in_traffic[str(event["attributes"]["http_path"])][param] = \
                        list(set(self.params_captured_in_traffic[str(event["attributes"]["http_path"])][param]))
                except TypeError:
                    pass
...
def main():
    ...
    os.system("autopep8 --in-place --aggressive --aggressive tests/*")
    print("\n\t\t..........done")
