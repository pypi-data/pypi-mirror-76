...
from ..utils import (
    ...
    get_network_file,
    get_ext_from_url,
    extract_video_thumb_and_duration
)
...
class NewsPostman(object):
    ...
    def _video_send_policy(self, url, data):
        ...
        if self._download_and_send:
            ...
            has_thumb, duration = extract_video_thumb_and_duration(video_full_name, thumb_full_name)
            if has_thumb:
                data['files'][thumb_name] = open(thumb_full_name, 'rb')
                return f'attach://{video_name}', f'attach://{thumb_name}', duration
            return f'attach://{video_name}', '', duration
    ...
    def _data_format(self, item, news_id):
        ...
        if ('images' in item and item['images']) or ('videos' in item and item['videos']):
            if len(item['images']) == 1 and len(item['videos']) == 0:
                ...
                method = 'sendVideo'
                data['caption'] = data.pop('text')
                data['video'], data['thumb'], data['duration'] = self._video_send_policy(item['videos'][0], data)
                ...
                for video in item['videos']:
                    media, thumb, duration = self._video_send_policy(video, data)
                    data['media'].append({
                        'type': 'video',
                        'media': media,
                        'thumb': thumb,
