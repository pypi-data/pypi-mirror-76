...
class NewsPostman(object):
    ...
    def _get_full(self, url, item):
        url = 'https://news.rthk.hk/rthk/ch/component/k2/1542949-20200810.htm'
        url = 'https://news.rthk.hk/rthk/ch/component/k2/1543101-20200811.htm'
        url = 'https://www.zaobao.com.sg/realtime/world/story20200811-1076096'
        text = ""
        if url:
    ...
    def _video_send_policy(self, url, data):
        ...
        if self._download_and_send:
            ...
            if extract_video_thumb(video_full_name, thumb_full_name):
                data['files'][thumb_name] = open(thumb_full_name, 'rb')
                return f'attach://{video_name}', f'attach://{thumb_name}'
            return f'attach://{video_name}', ''
    ...
    def _data_format(self, item, news_id):
        ...
        if ('images' in item and item['images']) or ('videos' in item and item['videos']):
            if len(item['images']) == 1 and len(item['videos']) == 0:
                ...
                method = 'sendVideo'
                data['caption'] = data.pop('text')
                data['video'], data['thumb'] = self._video_send_policy(item['videos'][0], data)
                ...
                for video in item['videos']:
                    media, thumb = self._video_send_policy(video, data)
                    data['media'].append({'type': 'video', 'media': media, 'thumb': thumb, 'supports_streaming': True})
