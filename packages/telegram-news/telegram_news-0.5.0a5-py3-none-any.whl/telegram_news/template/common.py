...
class NewsPostman(object):
    ...
    _attach_number = 0
    _attachments_dir = os.getcwd() + '\\attachments'
    ...
    def _file_send_policy(self, url, data):
        ...
        if self._download_and_send:
            ...
            download_file_by_url(url, self._attachments_dir + '\\' + name)
            data['files'][name] = open(self._attachments_dir + '\\' + name, 'rb') # get_network_file(url)
