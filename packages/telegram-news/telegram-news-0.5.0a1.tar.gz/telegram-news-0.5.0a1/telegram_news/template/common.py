...
class NewsPostman(object):
    ...
    def enable_download_and_send(self, enable=True):
    ...
    def _file_send_policy(self, url, data):
        ...
        if self._download_and_send:
            print('download_and_send')
            name = hashlib.md5(url.encode('utf-8')).hexdigest()
            download_file_by_url(url, 'attachments/' + name)
            data['files'][name] = open('attachments/' + name, 'rb') # get_network_file(url)
