...
def save(ctx, message, local, abort_merge):
    ...
    If you don't want to solve any merge conflicts rightaway run ZZ

        $ gt save --abort-merge

    This will commit any new changes and files and if a merge conflict appears after running git pull, gitutor will undo the git pull so that the merge conflict disappears.
...
def print_abort_merge_instructions():
        click.echo('\nLearn how to resolve conflicts here: https://gitutor.io/guide/multiple-users.html#saving-changes')
