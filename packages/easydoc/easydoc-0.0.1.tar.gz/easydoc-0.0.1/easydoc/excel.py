...
class EasyExcel:
    ...
    def write_data_to_excel(cls, content_list, header_list=[], header_row_index=1, header_column_index=1,
        ...
        for row in range(len(content_list)):
            if content_height:
                ws.row_dimensions[header_row_index + row].height = content_height
    ...
    def write_data_to_excel_autothreading(cls, content_list, header_list=[], header_row_index=1, header_column_index=1,
        ...
        full_file_name = cls.generate_file_name(file_dir, file_name)
        # start_account=content_list_size if content_list_size < cls.DEFAULT_DATA_SIZE else cls.DEFAULT_DATA_SIZE
        # for row in range(start_account):
        #     if content_height:
        #         ws.row_dimensions[header_row_index + row].height = content_height
        #     ws.append(content_list[row])
        ...
        for i in range(thread_count):
            ...
            if (i+1)==thread_count:
                # args.append(([full_file_name,wb,ws,content_list[start_account:content_list_size],start_account+header_row_index,header_column_index,content_height,alignment],None))
                pool.submit(cls.write_data_to_file,ws,content_list,start_account+header_row_index,content_list_size,header_row_index,header_column_index,content_height)  # 样式补充
                # pool.map(cls.write_data_to_file,[full_file_name],[content_list[start_account:]],[start_account],[header_column_index],[content_height])  # 样式补充
                # pool.submit(lambda parm: cls.write_data_to_file(*parm),[full_file_name,content_list[start_account:],start_account,header_column_index,content_height])  # 样式补充
            else:
                # args.append(([full_file_name,wb,ws,content_list[start_account:(start_account+cls.DEFAULT_DATA_SIZE)],start_account+header_row_index,header_column_index,content_height,alignment],None))
                pool.submit(cls.write_data_to_file,ws,content_list,start_account+header_row_index,start_account+cls.DEFAULT_DATA_SIZE,header_row_index,header_column_index,content_height)  # 样式补充
                # pool.map(cls.write_data_to_file,[full_file_name],[content_list[start_account:(start_account+cls.DEFAULT_DATA_SIZE)]],[start_account],[header_column_index],[content_height])  # 样式补充
                # pool.submit(lambda parm: cls.write_data_to_file(*parm),[full_file_name,content_list[start_account:(start_account+cls.DEFAULT_DATA_SIZE)],start_account,header_column_index,content_height])  # 样式补充

        # requests = threadpool.makeRequests(cls.write_data_to_file, args)
        # for req in requests:
        #     pool.putRequest(req)
        # pool.wait()

        # pool.submit(cls.write_data_to_excel,parm) for parm in args
    ...
    @classmethod
    def write_data_to_file(cls,ws,data_list,row_start_index,row_end_index):
        # wb = load_workbook(full_file_name)
        # ws = wb.active
        # ws = wb.get_active_sheet()

        while (row_start_index<=row_end_index):
            # col_size = len(data_list[row_start_index])
            # if content_height:
            #     ws.row_dimensions[row_start_index].height = content_height
            # for col in range(col_size):
            #     ws.cell(row=row_start_index + header_row_index, column=header_column_index + col).value = data_list[row_start_index][col]
            ws.append(data_list[row_start_index])
    ...
    @classmethod
    def write_data_to_excel_autothreading_style(cls, content_list, header_list=[], header_row_index=1, header_column_index=1,
        ...
        for i in range(thread_count):
            ...
            if (i + 1) == thread_count:
                ...
                pool.submit(cls.write_data_to_file_style, ws,
                                     content_list[start_account:content_list_size], start_account + header_row_index,header_row_index,
                                     header_column_index, content_height, alignment)  # 样式补充
                # pool.map(cls.write_data_to_file,[full_file_name],[content_list[start_account:]],[start_account],[header_column_index],[content_height])  # 样式补充
                # pool.submit(lambda parm: cls.write_data_to_file(*parm),[full_file_name,content_list[start_account:],start_account,header_column_index,content_height])  # 样式补充
            else:
                # args.append(([full_file_name,wb,ws,content_list[start_account:(start_account+cls.DEFAULT_DATA_SIZE)],start_account+header_row_index,header_column_index,content_height,alignment],None))
                pool.submit(cls.write_data_to_file_style,  ws,
                                     ...
                                     start_account + header_row_index, header_row_index,header_column_index, content_height,
                                     alignment)  # 样式补充
                # pool.map(cls.write_data_to_file,[full_file_name],[content_list[start_account:(start_account+cls.DEFAULT_DATA_SIZE)]],[start_account],[header_column_index],[content_height])  # 样式补充
                # pool.submit(lambda parm: cls.write_data_to_file(*parm),[full_file_name,content_list[start_account:(start_account+cls.DEFAULT_DATA_SIZE)],start_account,header_column_index,content_height])  # 样式补充
    ...
    @classmethod
    def write_data_to_file_style(cls, ws, data_list, row_start_index, header_row_index,header_column_index, content_height,
                           alignment):
        # wb = load_workbook(full_file_name)
        # ws = wb.active
        # ws = wb.get_active_sheet()
        ...
        for row in range(row_size):
            ...
            for col in range(col_size):
                ws.cell(row=row_start_index + row + header_row_index, column=header_column_index + col).value = data_list[row][col]
                if alignment:
                    ws.cell(row=row_start_index + row + header_row_index, column=header_column_index + col).alignment = alignment
        # wb.save(full_file_name)

    @classmethod
    def write_data_to_excel_multithreading(cls, content_list,thread_count, header_list=[], header_row_index=1, header_column_index=1,
                            header_height=30, content_height=None, file_dir=None, file_name=None, sheet_name=None,
                            column_width_dict={}, merge_value_dict={}):
        alignment = Alignment(horizontal="center", vertical="center", wrapText=True)
        content_list_size=len(content_list)
        ...
        for i in range(thread_count):
            ...
            if (i + 1) == thread_count:
                pool.submit(cls.write_data_to_file, ws, content_list, start_account + header_row_index,
                            content_list_size, header_row_index, header_column_index, content_height)
            else:
                pool.submit(cls.write_data_to_file, ws, content_list, start_account + header_row_index,
                            start_account + splice_size, header_row_index, header_column_index,
                            content_height)
    ...
    @classmethod
    def write_data_to_excel_multithreading_style(cls, content_list, thread_count,header_list=[], header_row_index=1, header_column_index=1,
                                          header_height=30, content_height=None, file_dir=None, file_name=None,
                                          sheet_name=None,
                                          column_width_dict={}, merge_value_dict={},
                                          alignment=Alignment(horizontal="center", vertical="center", wrapText=True)):
        ...
        for i in range(thread_count):
            ...
            if (i + 1) == thread_count:
                pool.submit(cls.write_data_to_file_style, ws,
                            content_list[start_account:content_list_size], start_account + header_row_index,
                            header_row_index,header_column_index, content_height, alignment)
            else:
                pool.submit(cls.write_data_to_file_style, ws,content_list[start_account:(start_account + splice_size)],
                            start_account + header_row_index, header_row_index, header_column_index, content_height,alignment)
...
if __name__ == '__main__':
    # file_name = EasyExcel.write_data_to_excel([['张学友', 15201062100, 18, '测试数据！'],
    #                                            ['李雷', 15201062598, 19,
    #                                             '测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！'],
    #                                            ['Marry', 15201062191, 28, '测试数据！']],
    #                                           header_list=['userName', 'Phone', 'age', 'Remark'],
    #                                           file_dir="/tmp/yangchangjia", file_name="report-action",
    #                                           column_width_dict={"A": 50, "B": 70, "C": 100, "D": 50})
    # print file_name

    # result=EasyExcel.read_data_form_excel("/tmp/yangchangjia/report-action-2020-08-14-12-08-45-159737812588653.xlsx",
    #                                min_row=1,max_row=4,min_col=1,max_col=4)
    # for line in result:
    #     print line
    ...
    content_list=[]
    for i in range(100):
      content_list.append(['李雷'+str(i), 15201062598, 19,'测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！测试数据！'])

    # start_time = time.time()
    # file_name = EasyExcel.write_data_to_excel_autothreading(content_list,
    #                                           header_list=['userName', 'Phone', 'age', 'Remark'],
    #                                           file_dir="/tmp/yangchangjia/", file_name="report-action",
    #                                           column_width_dict={"A": 20, "B": 40, "C": 20, "D": 50})
    # print file_name
    # end_time=time.time()
    # print end_time-start_time
    #
    # start_time = time.time()
    # file_name = EasyExcel.write_data_to_excel(content_list,
    #                                           header_list=['userName', 'Phone', 'age', 'Remark'],
    #                                           file_dir="/tmp/yangchangjia1/", file_name="report-action",
    #                                           column_width_dict={"A": 20, "B": 40, "C": 20, "D": 50})
    # print file_name
    # end_time = time.time()
    # print end_time - start_time
    ...
    # EasyExcel.write_data_to_excel_by_file("/tmp/test.xlsx",content_list,header_row_index=3,header_column_index=1,file_dir="/tmp/yangchangjia1/", file_name="report-action")
