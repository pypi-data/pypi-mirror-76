...
class Downloader:
    def __init__(self,query):
        ...
        self.folder_path = "downloaded/" # Master saving folder
        if not os.path.exists(self.folder_path + self.query.replace(" ","_")):
            os.makedirs(self.folder_path + self.query.replace(" ", "_"))
        
        self.sleep_time = 0.25 # Sleeping time
        self.number_of_request = 20 # Default number of request after end scrolling reach
            ...
            try:
                if img_url[0:4] == "data":
                    ...
                    if format not in self.supported_extensions:
                        print("Format ",str(format)," not supported")
                        continue
                    img_data = base64.b64decode(img_url)
                    file_name = self.folder_path+"house/"+self.query+"_"+str(self.img_count)+"."+str(format)
