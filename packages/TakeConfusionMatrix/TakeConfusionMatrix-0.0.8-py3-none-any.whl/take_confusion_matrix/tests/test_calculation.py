...
def test_matrix_image_returns_expected_results():
    ...
    ax = mc.generate_confusion_matrix(as_image=True)

    assert str(type(ax)) == "<class 'matplotlib.axes._subplots.AxesSubplot'>"


def test_matrix_with_duplicated_labels_retuns_expected_results():
    labels = [0, 1, 1]
    ...
    matrix = mc.generate_confusion_matrix()

    assert matrix.index.shape[0] == 2
    assert matrix.columns.shape[0] == 2
