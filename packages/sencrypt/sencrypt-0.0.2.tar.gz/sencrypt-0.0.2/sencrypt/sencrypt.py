...
def audit_encription():
    "Check encription of all s3 buckets"
    ...
    for bucket in s3.buckets.all():
        try:
            ...
            if e.response['Error']['Code'] == 'ServerSideEncryptionConfigurationNotFoundError':
                print("No serverside encription for ", bucket.name)
            ...
            if 'ServerSideEncryptionConfiguration' in default_encryption and 'Rules' in default_encryption['ServerSideEncryptionConfiguration']:
                print("Encription applied for the bucket", bucket.name, end = ": ")
