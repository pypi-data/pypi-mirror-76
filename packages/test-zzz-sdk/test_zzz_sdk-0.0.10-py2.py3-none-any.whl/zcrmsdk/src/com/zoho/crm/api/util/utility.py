...
class Utility(object):
    ...
    def get_fields(module_api_name):
        ...
        with lock:
            ...
            # if not (os.path.exists(record_field_details_path)) and Utility.search_json_details(module_api_name) is not None:
            #     return
            ...
            if os.path.exists(record_field_details_path):
                ...
                if Initializer.get_initializer().auto_refresh_fields and not Utility.new_file and not Utility.get_modified_modules:
                    ...
                    Utility.modify_fields(record_field_details_path, record_field_details_json[Constants.FIELDS_LAST_MODIFIED_TIME] if Constants.FIELDS_LAST_MODIFIED_TIME in record_field_details_json else None)
    ...
    @staticmethod
    def get_file_name():
        return os.path.join(Initializer.get_initializer().resource_path, Constants.FIELD_DETAILS_DIRECTORY, Converter.get_encoded_file_name())
    ...
    def get_related_lists(related_module_name, module_api_name, common_api_handler):
        try:
            ...
            if not os.path.exists(record_field_details_path) or (os.path.exists(record_field_details_path) and key not in Initializer.get_json(record_field_details_path)):
                is_new_data = True
    ...
    @staticmethod
    def fill_data_type():
        field_api_names_string = ["textarea", "text", "website", "email", "phone", "mediumtext", "multiselectlookup", "profileimage"]
    ...
    @staticmethod
    def set_data_type(field_detail, field_instance, module_api_name):
        api_type = field_instance.get_data_type()
        module = ''
        key_name = field_instance.get_api_name()
        ...
        elif key_name.lower() == Constants.PARTICIPANT_API_NAME.lower() and (module_api_name.lower() == Constants.EVENTS.lower() or module_api_name.lower() == Constants.ACTIVITIES.lower()):
            field_detail[Constants.NAME] = key_name
            field_detail[Constants.TYPE] = Constants.LIST_NAMESPACE
        ...
        if field_instance.get_pick_list_values() is not None and len(field_instance.get_pick_list_values()) > 0:
            values = list(map(lambda x: x.get_actual_value(), field_instance.get_pick_list_values()))

            field_detail[Constants.VALUES] = values

        if api_type == Constants.SUBFORM:
            module = field_instance.get_subform().get_module()

            field_detail[Constants.MODULE] = module
        if api_type == Constants.LOOKUP:
            module = field_instance.get_lookup().get_module()
