...
class JSONConverter(Converter):
    ...
    def is_record_request(self, record_instance, class_detail):
        ...
        request_json, module_detail, reset_modified = {}, {}, {}
        module_api_name = self.common_api_handler.module_api_name
        ...
        for key_name in key_values.keys():
            ...
            reset_modified[key_name] = 0
            request_json[key_name] = json_value

        setattr(record_instance, self.construct_private_member(class_name, Constants.KEY_MODIFIED), reset_modified)
    ...
    def is_not_record_request(self, request_instance, class_name, class_detail, instance_number):
        ...
        for member_name, member_detail in class_detail.items():
            ...
            if modification is not None and not modification == 0 and self.value_checker(class_name=class_name,
                ...
                try:
                    getattr(request_instance, Constants.SET_KEY_MODIFIED)(key_name, 0)
                except Exception as e:
                    JSONConverter.logger.info(Constants.EXCEPTION_IS_KEY_MODIFIED + e.__str__())
                ...
                if isinstance(request_instance, FileDetails):
                    if key_name.lower() == Constants.ATTACHMENT_ID.lower():
                        ...
                        if field_value is None:
                            request_json[key_name.lower()] = None
                        else:
                            request_json[key_name.lower()] = field_value
                else:
    ...
    def is_record_response(self, response_json, class_detail, pack):
        ...
        for each_class in class_list:
            if Constants.RECORD_TYPE in str(each_class):
                record_instance = JSONConverter.__get_instance_from_name(Constants.RECORD_NAMESPACE)
                class_name = Constants.RECORD_NAMESPACE.rpartition('.')[-1]
                break
