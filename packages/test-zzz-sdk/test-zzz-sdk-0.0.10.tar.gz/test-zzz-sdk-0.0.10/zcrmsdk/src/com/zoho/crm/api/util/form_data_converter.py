...
class FormDataConverter(Converter):
    ...
    def form_request(self, request_instance, pack, instance_number):
        ...
        for member_name in class_detail:
            ...
            if (Constants.READ_ONLY in member_detail and bool(member_detail[Constants.READ_ONLY])) or Constants.NAME not in member_detail:
                continue
            ...
            if modification is not None:
                try:
                    getattr(request_instance, Constants.SET_KEY_MODIFIED)(member_name, 0)
                except Exception as e:
                    FormDataConverter.logger.info(Constants.EXCEPTION_SET_KEY_MODIFIED + e.__str__())
