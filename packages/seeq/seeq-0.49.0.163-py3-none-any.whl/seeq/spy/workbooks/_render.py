...
def pull(report: _annotation.Report, *, errors='raise', quiet=False, status: Status = None):
    ...
    def _get_screenshot(_html_dom_id, _content_id):
        _timer = _common.timer_start()

        # noinspection PyBroadException
        try:
            status.send_update(_html_dom_id, {
                'Result': 'Rendering',
                'Time': _common.timer_elapsed(_timer)
            })

            if _config.options.pull_rendered_content_fresh:
            ...
            return _html_dom_id, _login.pull_image(_request_url)
    ...
    def _on_success(_row_index, _job_result):
        _html_dom_id, _image = _job_result
        images[(_html_dom_id, 'Content.png')] = _image

    img_matches = re.finditer(r'<img[^>]* id="([^"]+)".*?>', report.html, re.IGNORECASE)
    for img_match in img_matches:
        img_html = img_match.group(0)
        html_dom_id = img_match.group(1)
        content_id = _common.get_html_attr(img_html, 'data-seeq-content')

        status.df.at[html_dom_id, 'Content ID'] = content_id
        status.df.at[html_dom_id, 'Result'] = 'Queued'

        status.add_job(html_dom_id,
                       (_get_screenshot, html_dom_id, content_id),
                       _on_success)
