...
def _get_best_params(hyperopt, params, int_params, trials):
...
def _evaluation_plot(losses, best='min', invert=False):
...
def opt_catboost(X_train, y_train, cat_features=None, cv_folds=3, n_trials=20, verbose=0, params=None, device='GPU', max_rounds=5000, seed=0, early_stopping=20):
    ...
    # Plot results
    if verbose == 2:
        _evaluation_plot(trials.losses())

    return {'params':best_params, 'n':best_n}


def opt_xgboost(X_train, y_train, cat_features=None, cv_folds=3, n_trials=20, verbose=0, params=None, device='GPU', max_rounds=5000, seed=0, early_stopping=20):
    """
...
def opt_lightgbm(X_train, y_train, cat_features=None, cv_folds=3, n_trials=20, verbose=0, params=None, device='CPU', max_rounds=5000, seed=0, early_stopping=20):
    """
    ...
    Outputs:
        dict that contains the best params and the best number of iterations
    """
    HYPEROPT_PARAMS = {'objective': 'tweedie',
                                'tweedie_variance_power': hp.uniform('tweedie_variance_power', 1.0, 2.0),
                                'learning_rate': hp.loguniform('learning_rate', np.log(0.01), np.log(0.3)),
                                'num_leaves': hp.choice('num_leaves', [2 ** i - 1 for i in range(3, 13)]),
                                'max_depth': hp.quniform('max_depth', 2, 10, 2),  # 10,
                                'min_data_in_leaf': hp.choice('min_data_in_leaf', [2 ** i - 1 for i in range(3, 13)]),
