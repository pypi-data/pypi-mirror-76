...
def get_lmp(name,block1,block2,cageobj,BOSSdir,BABELexe,LMPexe):
    ...
    if cage_mol.HasSubstructMatch(patt):
        atomids = cage_mol.GetSubstructMatches(patt)
    ...
    print("units real\nboundary f f f\natom_style full\nbond_style harmonic\nangle_style harmonic\ndihedral_style opls\n"
          "improper_style harmonic\npair_style lj/cut/coul/cut 14.0 14.0\nspecial_bonds lj/coul 0.0 0.0 0.5\n"
          "pair_modify mix geometric",file=f)
    print('read_data "'+name+'.data"', file=f)
    print('neighbor 4 bin\nneigh_modify every 1 delay 0 check yes', file=f)
    ...
    BashCall("rm log.lammps {0}.data {0}.in dump.lammpstrj log".format(name))
