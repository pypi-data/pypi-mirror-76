			...
			for table, table_metadata in model.model_sources.items():
				...
				if table_metadata['source_type'] in ['other_model', 'other_staging_model', 'staging_model']:
					...
					if not table in self.table_index.keys():
						...
						try:
							description = self.sources_conf[self.table_index[table]['source_name']]['tables'][self.table_index[table]['table_short_name']]['description']
							self.table_index[table]['description'] = description
						except KeyError:
							pass
						self.table_index[table]['included_in'] = []
					self.table_index[table]['included_in'].append(model_name)
