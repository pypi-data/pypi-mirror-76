...
class ModelEngine:
  ...
  def source(self, source_name, table_name):
    ...
    if source_name == self.model:
      ...
      try:
        source_schema = self.sources_conf[source_name]['schema']['name']
        source_type = 'source'
      except KeyError:
    ...
    if source_name in self.sources_conf.keys():
      return f"{self.sources_conf[source_name]['schema']['name']}.{table_name}"
    else:
      return source_table
  ...
  def _render_model(self):
    for path in self._model_queries_path:
      ...
      if 'description' in self.model_config.keys():
        if table_short_name in self.model_config['description'].keys():
          self.rendered_model[table_name]['description'] = self.model_config['description'][table_short_name]
