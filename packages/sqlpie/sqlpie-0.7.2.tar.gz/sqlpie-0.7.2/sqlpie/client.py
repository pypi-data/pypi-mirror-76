...
class Sqlpie:
  ...
  #include in client only selected models
	...
	def build_table_index(self):
		for model_name, model in self.models.items():
			for table, table_metadata in model.rendered_model.items():
				self.table_index[table] = self.convert_model_metadata(model_name, table, table_metadata)
		...
		for model_name, model in self.models.items():
			for table, table_metadata in model.model_sources.items():
				if table_metadata['source_type'] in ['model', 'staging_model']:
					#add unknown source error handling
					self.table_index[table]['included_in'].append(model_name)
				elif table_metadata['source_type'] in ['other_model', 'other_staging_model']:
					self.table_index[table]['included_in'].append(model_name)
	...
	def convert_model_metadata(self, model_name, table_name, table_metadata):
		return {
