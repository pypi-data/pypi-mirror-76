...
class UpdateProcessTicketWithMMErrorTestCase(unittest.TestCase):
    ...
    def test_run(self, MockDF, MockMMErrorArticle, MockOTRSClient):
        ...
        def mock_MM_error_article_side_effect(error, builded_object):
            if error == self.expected_error and builded_object == "OrderItem":
        ...
        UpdateProcessTicketWithMMError(self.ticket_id, self.expected_error, self.object_str).run()
