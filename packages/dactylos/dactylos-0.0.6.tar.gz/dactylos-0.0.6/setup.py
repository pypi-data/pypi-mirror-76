...
class CMakeBuild(build_ext):
    ...
    def move_output(self, ext):
        ...
        build_temp = Path(self.build_temp).resolve(ext.name)
        print (f'Found build dir {build_temp}')        
        # FIXME - we use get_ext_fullpath to figure out the last bit of the filename
        # (this is the ARCH specific ending
        #print (Path(self.get_ext_fullpath(ext.name)).resolve())
        source_file = os.path.join(build_temp, ext._cfilename)
        print (f'Looking for source file {source_file}' + '*.so')
        source_files = glob(source_file + "*")
        if not source_files:
            raise SystemError("Can not find source file!")
        source_file = source_files[0]
        dest_path = Path(os.path.split(os.path.split(Path(self.get_ext_fullpath(ext._cfilename)).resolve())[0])[0]) / 'dactylos'

        print (f"Trying to copy {ext.name} from {source_file} to {dest_path}")
        self.copy_file(source_file, dest_path)

        ###print (f'Found source file {source_file}')
        ###file_ending = os.path.split(Path(self.get_ext_fullpath(ext._cfilename)).resolve())[1]
        ###print (f'Got cfilename {Path(self.get_ext_fullpath(ext._cfilename)).resolve()}')
        ###print (f'Identified file ending.. {file_ending}')
        ...
        ###except Exception as e:
        ###    print (e)
        ###source_filename = ext._cfilename
        ###if not source_filename.endswith('.so'):
        ###    # this the is a c++ extension python module, build with 
        ###    # pybindings. The file_ending is the real filename
        ###    # FIXME
        ###    source_filename = file_ending
        ###source_path = build_temp / source_filename
        ###print (f'Found source path {source_path}')
        ###print (f'Identified destination {dest_path}')
        ...
        ###    dest_directory = dest_directory / 'analysis' / 'shaping'
