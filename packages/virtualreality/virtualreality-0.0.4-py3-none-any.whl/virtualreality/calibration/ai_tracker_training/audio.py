...
def play_audio(array: np.ndarray, config: InputConfig, loop=False):
    ...
    p = pyaudio.PyAudio()
    stream = p.open(rate=config.sampling_rate, format=pyaudio.paFloat32, channels=channels, output=True)
    arrby = array.tobytes()
    stream.write(arrby)
    while loop:
        stream.write(arrby)
...
def get_audio_thread(music: Union[str, pathlib.Path], loop=False):
    ...
    audio_thread = multiprocessing.Process(target=play_audio, args=(np_file, audio_cfg, loop))
    return audio_thread
