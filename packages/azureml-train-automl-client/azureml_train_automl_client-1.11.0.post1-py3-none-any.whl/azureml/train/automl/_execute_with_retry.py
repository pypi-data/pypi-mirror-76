...
class ExecuteWithRetry:
    ...
    def __init__(self, max_retries, max_back_off_seconds, should_retry_func, log_message, action):
        ...
        if not callable(should_retry_func):
            raise ArgumentException('should_retry_func: Argument is not callable',
                                    target="should_retry_func",
                                    reference_code="_execute_with_retry.__init__.type_error",
                                    has_pii=False)
    ...
    def execute(self, func, func_name, *args, **kwargs):
        ...
        if not callable(func):
            raise ArgumentException('Argument is not callable',
                                    target="func",
                                    reference_code="_execute_with_retry.execute.type_error",
                                    has_pii=False)
