...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import AutoMLInternal, \
    ...
    InvalidOperationOnRunState, OnnxNotEnabled, OnnxSplitsNotEnabled, NoMetricsData
...
class AutoMLRun(Run):
    ...
    def _fail_with_error(self, exception: BaseException) -> None:
        ...
        if isinstance(exception, AzureMLServiceException):
            ...
            if not isinstance(exception, AzureMLException):
                # Wrap unknown exception types into an AutoMLInternal error
                logger.warning("Encountered an exception of type: {}. Failing the run with a SystemError.".
                               format(type(exception)))
                error = AzureMLError.create(AutoMLInternal, error_details=str(exception))
                exception = ClientException(azureml_error=error, inner_exception=exception)
