...
def ForumCode_Submit(assignment=None, guest = False, websubmit=False): #TODO: Code websubmit
    """
    ...
    xhttp.send("rand="""+str(rand_code)+"""&url=".concat(btoa(notebook_url),"&data=",notebook_url));
    """
    display(Javascript(js_submission_code))

    import requests
    time.sleep(5)
    r = requests.get(base_url + "/ForumCode/check/" + str(rand_code))
    if (str(r.content, 'utf-8', 'ignore')[0] == "Y"):
        display(HTML(
            """<div class=\"alert alert-block alert-success\"><b>Upload successful!</b> You can view your submitted notebook on <a target=\"_blank\" href=\"{0}\" class=\"alert-link\"> this URL.</a>\n </div>""".format(
                str(r.content, 'utf-8', 'ignore')[1:])))
        display(Javascript(skins.JS_PROGRESS_BAR_FINAL))
    else:
        display(HTML(skins.HTML_UPLOAD_ERROR_CUSTOM_START + str(r.content, 'utf-8',
                                                                'ignore') + skins.HTML_UPLOAD_ERROR_CUSTOM_END))
