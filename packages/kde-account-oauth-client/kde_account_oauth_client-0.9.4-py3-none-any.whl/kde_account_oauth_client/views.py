...
def kde_account_oauth_settings() -> ClientSettings:
    """Container for KDE Account OAuth Client settings."""
    from urllib.parse import urljoin

    url_base = settings.KDEACCOUNT['BASE_URL']
    return ClientSettings(
        client=settings.KDEACCOUNT['OAUTH_CLIENT'],
        secret=settings.KDEACCOUNT['OAUTH_SECRET'],
        url_base=url_base,
...
def login_view(request):
    """Login the user by redirecting to KDE Account.
    ...
    """
    kde_account_oauth = kde_account_oauth_settings()
    redirect_uri = request.build_absolute_uri(reverse('oauth:callback'))
    bid = OAuth2Session(kde_account_oauth.client, redirect_uri=redirect_uri)
    authorization_url, state = bid.authorization_url(kde_account_oauth.url_authorize)
...
def callback_view(request):
    ...
    bid = OAuth2Session(kde_account_oauth.client,
                        state=session_state,
                        redirect_uri=redirect_uri)

    try:
        token = bid.fetch_token(kde_account_oauth.url_token, client_secret=kde_account_oauth.secret,
        ...
        # This happens if the user takes too long to log in; KDE Account has
        # then already discarded the grant, and we should just try again.
        my_log.info('%s error received from KDE Account, retrying the login', type(ex))
        return redirect_to_login(next_after_login)
    else:
        request.session['bid_login_retry_count'] = 0

    bid = OAuth2Session(kde_account_oauth.client, token=token)
    resp = bid.get(kde_account_oauth.url_userinfo)
    if resp.status_code != 200:
        my_log.error('Error %s getting user info URL %s with token %s: %s',
                     resp.status_code,
                     kde_account_oauth.url_userinfo,
                     bid.token,
                     resp.text,
                     )
        # TODO(Sybren): create a proper page for this error, or at least style it nicely.
        return HttpResponseServerError('Error %d getting your user information from KDE Account'
                                       % resp.status_code)
    ...
    # Update the user with the info we just got from KDE Account.
    # TODO(Sybren): handle nickname conflict
    # TODO(Sybren): handle email conflict
...
def logout_view(request: HttpRequest):
    """Logout from this site, and then from KDE Account.

    This helps perceiving the current site as part of KDE Account.
    """
    ...
    redirect_url = kde_account_oauth.url_logout
...
def login_loop_detected(request: HttpRequest):
    return render(request, 'kde_account_oauth_client/login_loop_detected.html')
