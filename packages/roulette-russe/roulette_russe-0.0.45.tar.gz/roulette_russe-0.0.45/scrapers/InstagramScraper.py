...
class InstagramScraper(Scraper):
    ...
    def _query_posts(self, cookies, id, end_cursor=''):
        req = self.session.get(
    ...
    def query_posts(self, cookies, id):
        posts, end_cursor = self._query_followers(cookies, id)
        ...
        if posts:
            while True:
                for post in posts:
                    yield post
                if end_cursor:
                    posts, end_cursor = self._query_posts(cookies, id, end_cursor)
                else:
                    return
    ...
    def scrap(self):
        ...
        if 'posts' in self.output_datasets:
            for account in tmp_list_profiles:
                posts_data = self.query_posts(self.cookies, account['internal_id'])
                tmp_list_posts.append(posts_data)
            self.output_datasets['posts'] = self.output_datasets['posts'].append(tmp_list_posts, ignore_index=True)
        if 'followers' in tmp_list_profiles:
            for account in self.output_datasets['profiles']:
                followers_data = self.query_followers(self.cookies, account['internal_id'])
                tmp_list_followers.append(followers_data)
            self.output_datasets['followers'] = self.output_datasets['followers'].append(tmp_list_followers, ignore_index=True)
        if 'following' in tmp_list_profiles:
            for account in self.output_datasets['profiles']:
                following_data = self.query_followings(self.cookies, account['internal_id'])
                tmp_list_following.append(following_data)
        ...
        if 'commenters' in self.output_datasets:
            for post in tmp_list_posts:
                ...
                tmp_list_commenters.append(commenters_data)
        ...
        if 'likers' in self.output_datasets:
            for post in tmp_list_posts:
                ...
                tmp_list_likers.append(likers_data)
