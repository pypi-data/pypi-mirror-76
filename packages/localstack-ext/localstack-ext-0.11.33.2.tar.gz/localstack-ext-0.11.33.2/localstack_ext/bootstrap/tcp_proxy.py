...
class AttrDict(jVqAm):
 def __init__(self,*args,**kwargs):
  jVqAy(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and jVqAi(socket,'IPV6_V6ONLY'):
...
class Half(jVqAC):
 def __init__(self,opt,sock,addr,jVqAQ):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.jVqAQ=jVqAQ
  self.name='peer' if self.jVqAQ else 'client'
  self.queue=[]
  self.dest=jVqAR
  self.err=jVqAR
  self.ready=jVqAa
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':jVqAa,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':jVqAa,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':jVqAa,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':jVqAa,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
