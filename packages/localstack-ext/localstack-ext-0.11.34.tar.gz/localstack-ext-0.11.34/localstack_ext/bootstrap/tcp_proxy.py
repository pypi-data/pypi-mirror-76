...
class AttrDict(vhpcr):
 def __init__(self,*args,**kwargs):
  vhpcW(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and vhpcy(socket,'IPV6_V6ONLY'):
...
class Half(vhpcf):
 def __init__(self,opt,sock,addr,vhpcM):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.vhpcM=vhpcM
  self.name='peer' if self.vhpcM else 'client'
  self.queue=[]
  self.dest=vhpca
  self.err=vhpca
  self.ready=vhpcJ
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':vhpcJ,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':vhpcJ,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':vhpcJ,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':vhpcJ,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
