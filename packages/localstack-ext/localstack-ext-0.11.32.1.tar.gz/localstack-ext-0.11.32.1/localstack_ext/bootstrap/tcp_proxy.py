...
class AttrDict(YmoKM):
 def __init__(self,*args,**kwargs):
  YmoKS(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and YmoKC(socket,'IPV6_V6ONLY'):
...
class Half(YmoKV):
 def __init__(self,opt,sock,addr,YmoKR):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.YmoKR=YmoKR
  self.name='peer' if self.YmoKR else 'client'
  self.queue=[]
  self.dest=YmoKN
  self.err=YmoKN
  self.ready=YmoKa
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':YmoKa,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':YmoKa,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':YmoKa,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':YmoKa,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
