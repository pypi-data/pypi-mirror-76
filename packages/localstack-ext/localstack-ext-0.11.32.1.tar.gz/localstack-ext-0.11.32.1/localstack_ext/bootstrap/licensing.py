...
def read_api_key():
 ...
 if key:
  return key
 raise JpAfM('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=JpAfY)
 finally:
