...
def xmind_to_zentao_csv_file(xmind_file):
    ...
    fileheader = ["所属产品", "所属模块", "用例标题", "前置条件", "步骤", "预期", "关键词", "优先级", "用例类型", "用例状态", "适用阶段"]
...
def gen_a_testcase_row(testcase_dict):
    ...
    # 默认用例的状态是未执行。
    case_state = '未执行'
    row = [case_product, case_module, case_title, case_precontion, case_step, case_expected_result, case_keyword,
           case_priority, case_type, case_state, case_apply_phase]
    return row
