...
from enhancedlabelme.widgets import EscapableQListWidget
from enhancedlabelme.widgets import LabelDialog
from enhancedlabelme.widgets import LabelQListWidget
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.setWindowTitle(__appname__ + ' ' + __version)
        ...
        help = action(self.tr('&About'), self.about, icon='help',
                      tip=self.tr('About LabelMe-Modified'))
        ...
        darkMode = action(self.tr('&Toggle dark theme'), self.darkModeToggle,
    ...
    def preference(self):
        if platform.system() == 'Darwin':       # macOS
            subprocess.call(('open', os.path.join(os.path.expanduser('~'), 'labelmeconfig')))
        elif platform.system() == 'Windows':    # Windows
            subprocess.call(('notepad.exe', os.path.join(os.path.expanduser('~'), 'labelmeconfig')))
            #tempwindow = 'notepad.exe ' + os.path.join(os.path.expanduser('~'), 'labelmeconfig')
            #os.startfile(tempwindow)
        else:                                   # linux variants
            subprocess.call(('xdg-open', os.path.join(os.path.expanduser('~'), 'labelmeconfig')))

    def about(self):
        f01 = "\n- Added some keys to json data (['loadTime'], ['saveTime']"
        f02 = '\n- Removed double click function when creating polygon'
        f03 = '\n- Always dump json in utf-8 encoding'
        f04 = '\n- Automatically check "Save Automatically"'
        f05 = '\n- Added Progress indication in the Main window title : (# of images/# of jsons)'
        f06 = '\n- Added the Preference in menubar (File - Preference)'
        f07 = '\n- Launch program in fullscreen mode'
        f08 = '\n- Added dark theme (Edit - Toggle dark theme)'
        return QtWidgets.QMessageBox.about(
            self, 'About', '\n<Version %s>%s'%(__version__, f01 + f02 + f03 + f04 + f05 + f06 + f07 + f08))
