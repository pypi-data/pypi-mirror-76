...
class git:
    ...
    def push_all_files(disk,path,message):
        os.system(f'cmd /k "{disk}: && cd {path} && git pull && git add . && git commit -m "{message}" && git push origin master"')
    def push_one_file(disk,path,filename,message):
        os.system(f'cmd /k "{disk}: && cd {path} && git pull && git add {filename} && git commit -m "{message}" && git push origin master"')
