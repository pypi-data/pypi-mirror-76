...
if __name__ == '__main__':
    ParallelCorpus.set_restrict_show(False)

    en = ParallelCorpus(
        'я получить', 1,
        marker=str.upper,
        subcorpus=Subcorpus.Parallel.English
    )
    en.open_url()

# TODO: DOCS
# при указании маркера не вызывайте его как функцию
# запрос в параллельный корпус может быть как на русском языке, так и на английском.
# если работаете с файлом, то в корпус достаточно передавать его имя.
# запрос нескольких слов (словарная форма) одной строкой через пробел,
#  тогда дистанция между словами ставится по умолчанию (Corpus.__MIN, Corpus.__MAX).
#  Её можно изменить (Corpus.set_min(value), Corpus.set_max(value)
#  до инициализации экземпляра)
# передавайте пустую строку в качестве параметров слова, если не указываете их
