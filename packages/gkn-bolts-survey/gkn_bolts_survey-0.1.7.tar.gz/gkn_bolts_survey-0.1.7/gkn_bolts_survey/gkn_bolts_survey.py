...
def signal_approximation(signal, learing_rate=0.2, max_iter=100):
    """
    ...
    sigmoid_approximation(signal, learing_rate=0.2, max_iter=100)
...
def plot_survey(Signals, threshold_torque=0.0, target=None, 
    ...
    if target is not None and target.strategy == 'torque_controlled':
        x0 = 2.0
    ...
    if target is not None and target.strategy == 'angle_controlled':
    ...
    if target is None:
        ...
        r'$T_y = %i Nm$'%Ty,
        r'$\alpha_y = %i deg$'%Ay))
    elif target.strategy == 'torque_controlled':
        textstr = '\n'.join((
        r'torque',
        r'controlled',
        r'$T_y = %i Nm$'%Ty))
