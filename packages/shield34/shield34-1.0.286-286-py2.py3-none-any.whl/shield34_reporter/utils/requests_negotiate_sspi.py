...
class HttpNegotiateAuth(AuthBase):
    ...
    def _retry_using_http_Negotiate_auth(self, response, scheme, args):
        ...
        try:
            ...
            request.headers['Proxy-Authorization'] = '{} {}'.format(scheme, base64.b64encode(auth[0].Buffer).decode('ASCII'))
    ...
    def _response_hook(self, r, **kwargs):
        if r.status_code == 407:
            for scheme in ('Negotiate', 'NTLM', 'KERBEROS'):
                if scheme.lower() in r.headers.get('Proxy-Authenticate', '').lower():
