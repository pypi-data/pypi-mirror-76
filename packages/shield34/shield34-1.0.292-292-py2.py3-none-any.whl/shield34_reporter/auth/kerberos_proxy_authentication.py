...
def authenticate_proxy(auth_info, response, **args):
    context = HTTPKerberosAuthContext()
    try:
        context.pos = response.request.body.tell()
        ...
        # None.
        context.pos = None
    HTTPKerberosAuth.handle_response(context, response, **args)
...
class HTTPKerberosAuth:
    ...
    @staticmethod
    def generate_request_header( context,response, host, is_preemptive=False):
        """
        ...
        try:
            ...
            kerb_host = context.hostname_override if context.hostname_override is not None else host
            kerb_spn = "{0}@{1}".format(context.service, kerb_host)
            print(kerb_spn)
            result, context.context[host] = kerberos.authGSSClientInit(kerb_spn,
                gssflags=gssflags, principal=context.principal)
            ...
            if context.cbt_struct:
                result = kerberos.authGSSClientStep(context.context[host],
                                                    negotiate_resp_value,
                                                    channel_bindings=context.cbt_struct)
            else:
                result = kerberos.authGSSClientStep(context.context[host],
                                                    negotiate_resp_value)
            ...
            gss_response = kerberos.authGSSClientResponse(context.context[host])
    ...
    @staticmethod
    def authenticate_user( response, **kwargs):
        ...
        try:
            auth_header = HTTPKerberosAuth.generate_request_header(response, host)
    ...
    def handle_407( response, **kwargs):
        ...
        if _negotiate_value(response) is not None:
            _r = HTTPKerberosAuth.authenticate_user(response, **kwargs)
    ...
    @staticmethod
    def handle_other(context, response):
        ...
        if context.mutual_authentication in (REQUIRED, OPTIONAL) and not context.auth_done:

            is_http_error = response.status_code >= 400

            if _negotiate_value(response) is not None:
                log.debug("handle_other(): Authenticating the server")
                if not HTTPKerberosAuth.authenticate_server(response):
            ...
            elif is_http_error or context.mutual_authentication == OPTIONAL:
                if not response.ok:
                    log.error("handle_other(): Mutual authentication unavailable "
                              "on {0} response".format(response.status_code))

                if(context.mutual_authentication == REQUIRED and
                       context.sanitize_mutual_error_response):
    ...
    def handle_response(context, response, **kwargs):
        ...
        if not context.cbt_binding_tried and context.send_cbt:
            ...
            if cbt_application_data:
                ...
                try:
                    context.cbt_struct = kerberos.channelBindings(application_data=cbt_application_data)
                except AttributeError:
                    # Using older version set to None
                    context.cbt_struct = None
            # Regardless of the result, set tried to True so we don't waste time next time
            context.cbt_binding_tried = True

        if context.pos is not None:
            # Rewind the file position indicator of the body to where
            # it was to resend the request.
            response.request.body.seek(context.pos)
        ...
        if response.status_code == 407 and num_407s < 2:
            ...
            _r = HTTPKerberosAuth.handle_407(response, **kwargs)
            log.debug("handle_response(): returning %s", _r)
            log.debug("handle_response() has seen %d 401 responses", num_407s)
            num_407s += 1
            return HTTPKerberosAuth.handle_response(_r, num_407s=num_407s, **kwargs)
            ...
            _r = HTTPKerberosAuth.handle_other(response)
            log.debug("handle_response(): returning %s", _r)
            return _r
