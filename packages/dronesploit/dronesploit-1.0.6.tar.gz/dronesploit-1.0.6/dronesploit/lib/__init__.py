...
class DronesploitConsole(FrameworkConsole):
    ...
    def interfaces(self):
        ...
        # TODO: add SDR interfaces collection here
        #       format: [None, None, mac]
        #                 ^     ^
        #     not considered   considered as connectionless for SDR
        #     as monitoring     unless a "paired" state can be defined
        return d.keys()
    
    @property
    def man_interfaces(self):
        return [i for i, x in self.state['INTERFACES'].items() \
                if isinstance(x[0], bool) and not x[0]]
    
    @property
    def mon_interfaces(self):
        return [i for i, x in self.state['INTERFACES'].items() \
                if isinstance(x[0], bool) and x[0]]
    
    @property
    def radio_interfaces(self):
        return [i for i, x in self.state['INTERFACES'].items() if x[0] is None]
    
    @property
    def self_mac_addresses(self):
        return [x[2] for x in self.state['INTERFACES'].values() \
                if isinstance(x[0], bool) and x[2] is not None]
