...
class Jaynes:
    ...
    def make_host_script(self, log_dir="~/debug-outputs", setup=None, terminate_after=False, delay=None,
                         instance_tag=None, region=None, **_):
        """
    ...
    def launch_manager(self, host, log_dir, project=None, user=None, token=None,
                       sudo=False, cleanup=True, verbose=False, **_):
        ...
        client = JaynesClient("http://localhost:8092")
        ...
        parent_dir = os.path.dirname(remote_script_name)
        status, stdout, stderr = client.execute(f"ls {parent_dir}")
        print(stdout, stderr)

        status, stdout, stderr = client.execute(f"bash {remote_script_name}")
        print(stdout, stderr)
...
def run(fn, *args, __run_config=None, **kwargs, ):
    ...
    # elif  launch_config['type'] == "manager" and j.host_unpack_script is None:
    #     j.host_unpack_script = j.k
