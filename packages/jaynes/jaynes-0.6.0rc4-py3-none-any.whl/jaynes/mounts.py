...
class Mount:
    ...
    def upload(self, host=None, verbose=None, ):
...
class TarMount(Mount):
    ...
    def __init__(self, local_path, local_tar=None, remote_tar=None, host_path=None, snapshot_path=None,
                 container_path=None,
                 pypath=False, name=None, excludes=None, file_mask=None, compress=True, servers=[]):
        self.local_path = local_path
        self.host_path = host_path
        self.snapshot_path = snapshot_path
        self.container_path = container_path or snapshot_path
        self.pypath = pypath
    ...
    def upload(self, host, verbose=None):
    ...
    def watch(self, sleep=None, initial=True):

        if initial:
            for s in self.servers:
                # these are named uploads
                s.upload(self.tar_path, self.name)

        while True:
            import time
            time.sleep(sleep)
            for s in self.servers:
                changed_file = "folder/some_file.py"
                s.upload(changed_file, self.name)
                print(f'uploaded {changed_file}...')
