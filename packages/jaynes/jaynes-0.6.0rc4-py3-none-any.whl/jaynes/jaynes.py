...
class Jaynes:
    ...
    def launch_manager(self, host, log_dir, project=None, user=None, token=None,
                       sudo=False, cleanup=True, verbose=False, ):
        ...
        host_setup = [dedent(m.host_setup) for m in self.mounts if hasattr(m, "host_setup") and m.host_setup]
        pipe_back = client.map(host_setup)
        print(*pipe_back, sep="\n")
        status, stdout, stderr = client.execute(remote_script_name)
        print(stdout)
...
def config(mode=None, *, config_path=None, runner=None, host=None, launch=None, **ext):
    ...
    RUN.J.upload_mount(host=RUN.config.get('host'), verbose=RUN.config.get('verbose'))
...
def run(fn, *args, __run_config=None, **kwargs, ):
    ...
    if launch_config['type'].startswith('ssh') and j.host_unpack_script is None:
        ...
        j.host_unpack_script = j.make_host_unpack_script(**host_config)
    ...
    # config.LAUNCH
    _ = getattr(j, launch_config['type'])(**host_config, **omit(launch_config, 'type'))
    if RUN.config.get('verbose'):
