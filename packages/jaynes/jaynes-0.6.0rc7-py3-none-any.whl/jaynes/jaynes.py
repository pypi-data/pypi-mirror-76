...
class Jaynes:
    ...
    def make_host_script(self, log_dir="~/debug-outputs", setup=None, terminate_after=False,
        ...
        tee_string = "tee -a " if tee else ""
        if tee:
            output = f"> >(tee -a {log_path}) 2> >(tee -a {error_path} >&2)"
        else:
            output = f"2> {error_path} 1> {log_path}"
...
{ec2_terminate(region, delay) if terminate_after else ""}
    ...
    def launch_manager(self, host, log_dir, project=None, user=None, token=None,
                       sudo=False, cleanup=True, blocking=True, verbose=False, timeout=None, **_):
        ...
        if not blocking:
            cmd += " &"
        status, stdout, stderr = client.execute(cmd, timeout)
        print(stdout, stderr)
