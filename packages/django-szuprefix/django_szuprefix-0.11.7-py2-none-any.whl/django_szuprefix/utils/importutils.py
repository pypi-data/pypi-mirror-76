...
FIELD_NAMES_MAP = {}
for f in [HanNameField, MobileField, IDCardField, QQField, WeixinIDField, EmailField, UseridField, PositionField]:
...
class BaseImporter(object):
    ...
    def from_model(self, model):
        smap = FIELD_NAMES_MAP
        fs = model._meta.get_fields()
        fields = [smap.get(f.verbose_name) for f in fs if not f.primary_key]
        return [(f.name, f.verbose_name, smap.get(f.verbose_name)) for f in fs if not f.primary_key]

class ManufacturerImporter(BaseImporter):
    stand_field = 'name'
    together_fields = ['name', 'manufacturer', 'number']
