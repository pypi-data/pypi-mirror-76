...
from django.forms.fields import MultiValueField, ChoiceField, Field
...
# class CompositeChoicesField(MultiValueField):
#     widget = CompositeChoicesWidget
#     default_error_messages = {
#     }
#
#     def __init__(self,  choice_set={}, **kwargs):
#         errors = self.default_error_messages.copy()
#         if 'error_messages' in kwargs:
#             errors.update(kwargs['error_messages'])
#         kwargs.pop("max_length",None)
#         self.keynames=[name  for name,choices in choice_set.iteritems() ]
#         fields = [ChoiceField(choices=choice_set[name])  for name  in self.keynames]
#         super(CompositeChoicesField, self).__init__(fields,  **kwargs)
#
#     def compress(self, data_list):
#         if data_list:
#             d=dict([(name,data_list[i]) for i,name in enumerate(self.keynames)])
#             print "%%%",d
#             return d
#         return {}
...
class CompositeChoicesField(Field):
    ...
    def validate(self, value):
        if not isinstance(value, (dict)):
            raise ValidationError(u"数据格式不正确", code='invalid')
...
class JsonField(Field):
    ...
    def to_python(self, value):
        ...
        if isinstance(value, widgetutils.BrokenData):
            raise ValidationError(u"Json格式不正确", code='invalid')
        # try:
