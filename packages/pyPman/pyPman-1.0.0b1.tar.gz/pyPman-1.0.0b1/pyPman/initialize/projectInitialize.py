...
def init_project(install_path, project=None):
    """
    Initializes the project with valid user inputs
    :param install_path: install path to gather and store user inputs
    :param project: Project name to work on, default in None
    :return: Returns the user inputs dictionary
    """
    global PROJECT_NAME, PACKAGE_NAME, PACKAGE_VERSION, DESCRIPTION, ENTRY_POINT, GIT_REPO
    global KEYWORDS, AUTHOR, AUTHOR_EMAIL, LICENSE, IS_GIT_SETUP
    ...
    if GIT_REPO:
        IS_GIT_SETUP = query_detail(
            "Is git repo already setup with PyProjectManager?",
            default=package_dict.get("is_pypm_setup", None) or IS_GIT_SETUP
        )
...
def query_detail(query, default=None, accepted=False):
    """
    Details will be enquired and returns the result of user
    :param query: Query to display on the CLI
    :param default: Displays default value as an optional value in CLI, default is None
    :param accepted: If this is True, bypasses the query and returns default value
    :return: returns user defined input or default value passed
    """
    query = query.strip() + ": "
    if default:
