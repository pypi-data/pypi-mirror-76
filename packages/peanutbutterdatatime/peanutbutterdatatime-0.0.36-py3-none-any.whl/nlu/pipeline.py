...
class NLUPipeline(BasePipe):
    ...
    def pythonify_spark_dataframe(self, processed, get_different_level_output=True, keep_stranger_features = True, stranger_features = [] ):
        '''
        ...
        final_cols = self.drop_irrelevant_cols(final_cols)
        ...
        try:
            if type(data) is  pyspark.sql.dataframe : # casting follows spark->pd
                if self.raw_text_column in data.columns:
                    ...
                    if len(data.columns) > 1 : stranger_features = list( set(data.columns) - set(self.raw_text_column))
