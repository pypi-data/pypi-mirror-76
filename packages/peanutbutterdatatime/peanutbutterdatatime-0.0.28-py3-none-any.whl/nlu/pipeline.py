...
class NLUPipeline(BasePipe):
    ...
    def pythonify_spark_dataframe(self, processed, get_different_level_output=True):
        ...
        # Or they are at a different level, in which case they will be left as is 
    ...
    def predict(self, data, output_level='', output_positions=False):
        ...
        try:
            if type(data) is  pyspark.sql.dataframe : # casting follows spark->pd
                if self.raw_text_column in data.columns: sdf = self.spark_transformer_pipe.transform(data )
                else :
            ...
            if type(data) is pd.DataFrame:  # casting follows pd->spark->pd
                ...
                if self.raw_text_column in data.columns: sdf = \
                    ...
                    self.spark.createDataFrame(data), )
                print('Could not find column named "text" in  Pandas Dataframe generated from input  Pandas Series. Please ensure one column named such exists. Columns in DF are : ', data.columns)
            ...
            # TODO type cast the pandas columns to string/array of string instead of objects! There should be no object type columns in final output
