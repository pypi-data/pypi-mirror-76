...
def load(request, verbose = False):
    ...
    try :         
        ...
        print("Something went wrong during loading and fitting the pipe. Check the other prints for more information and also verbose mode")
        return None
    return pipe
...
def construct_component_from_pipe_identifier(language, sparknlp_reference):
    ...
    for component in pipe.light_model.pipeline_model.stages:
        ...
        if parsed == 'document': constructed_components.append(nlu.Util(model=component)) # todo utils abuse
        if parsed == 'sentence': constructed_components.append(nlu.Util(model=component)) # todo utils abuse 
        if parsed == 'regex': constructed_components.append(nlu.Util(model=component)) # todo utils abuse
        ...
        if parsed == 'chunker': constructed_components.append(nlu.Classifier(model=component)) # todo utils abuse 
        if parsed == 'stopwords': constructed_components.append(nlu.Classifier(model=component)) # todo utils abse 
