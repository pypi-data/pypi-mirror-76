...
class GoogleCalendar():
    ...
    def create_or_update_event(self, title, date_start, date_end, timezone, description):
        # check if event already exists
        existing_event = self.event_exists(description)
    ...
    def update_event(self, existing_event, title, date_start, date_end, timezone, description):
        event_result = self.service.events().update(calendarId='primary',
                                                    ...
                                                    body={
                                                        ...
                                                        "end": {"dateTime": date_end.strftime('%Y-%m-%dT%H:%M:%SZ'), "timeZone": timezone} if hasattr(date_end, 'hour') else {'date': str(date_end+timedelta(days=1))},
                                                    }
                                                    ).execute()
    ...
    def create_event(self, title, date_start, date_end, timezone, description):
    ...
    def event_exists(self, description):
