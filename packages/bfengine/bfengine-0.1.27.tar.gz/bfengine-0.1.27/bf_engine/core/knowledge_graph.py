...
class KnowledgeGraph(Module):
    ...
    def train_by_path(self, data_path: str, is_new: bool) -> bool:
        """
        训练kg
        ...
        """
        try:
            self._upload_file(data_path, is_new)
            self._train()
            self._sync()
            return True
        except BfEngineException as bfe:
    ...
    def _upload_file(self, file_path, is_new):
        # log.info("kg: 开始上传训练数据")
        self.caller.load_file(file_path, is_new)
        # log.info("kg: 训练数据上传成功")
