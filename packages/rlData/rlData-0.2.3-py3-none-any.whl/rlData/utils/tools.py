...
class FactorApiGenerator(object):
    ...
    def __init__(self,group):
        self.__group = group
        self.__func = {}
        self.__func["list"] = self.__build_list(group)
        self.__func["query"] = self.__build_query(group)

        status, retobjs = getList(group)
        if status == 200:
            for _, row in retobjs.iterrows():
                self.__func[row['key']] = self.__build_api(group, row['key'])

    def __getattr__(self, name):
        if name in self.__func.keys():
