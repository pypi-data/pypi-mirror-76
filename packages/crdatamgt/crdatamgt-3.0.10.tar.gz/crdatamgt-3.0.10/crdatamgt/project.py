...
def project_load_from_workbooks(workbooks, **kwargs):
    ...
    for result in result_wb:
        ...
        log.info(f'Running Topic ID: {result_data.get("Summary").get("Topic ID").values[0]}')
        ...
        if not formulation_data.dropna(how='all').empty:
            ...
            log.warn(f"No Formulation data for Topic ID:{result_data.get('Summary').get('Topic ID').values}")
