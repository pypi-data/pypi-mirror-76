...
def get_maxmotion_lib(setupDir):
    ...
    #creates link in the launcher folder to the motion executable in the demo folder
    launcherfolder = setupDir + "/rocket/rocketlauncher"
    motion_demo_folder = setupDir + setupDir + "/rocket/maxmotion/demo/"
    os.system("ln -s %s %s" % (motion_demo_folder + "motion", launcherfolder))
    #creates link in the launcher folder to the motion configuration in the demo folder  
    ...
    if os.path.isdir(repo):
       print("Nothing to clone. %s already here." % (repo))   
        ...
        print("The example sources could not be cloned. Missing %s repo " %(repo))
...
def install_daisy(setupDir):
    ...
    os.chdir(setupDir)
    os.chdir("/rocket/rocketlauncher")
    os.system("cp -v blacklist.conf /etc/modprobe.d/blacklist.conf")
