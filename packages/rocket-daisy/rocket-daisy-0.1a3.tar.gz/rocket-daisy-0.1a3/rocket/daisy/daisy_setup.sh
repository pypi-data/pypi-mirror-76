...
if [ -z "$INSTALLED" ]; then
	if [ -z "$FOUND" ]; then
		echo "ERROR: Daisy cannot be installed - neither python or python3 found"
		exit 1
	else
...
if [ ! -f "/etc/daisy/config" ]; then
	...
	cp -v python/config /etc/daisy/config
fi
...
if [ ! -f "/etc/daisy/passwd" ]; then
	...
	mkdir /etc/daisy 2>/dev/null 1>/dev/null
	cp -v daisy/passwd /etc/daisy/passwd
fi

# Add service/daemon script
#if [ ! -f "/etc/init.d/daisy" ]; then
echo "Installing startup script..."
cp -rf python/daisy.init.sh /etc/init.d/daisy
sed -i "s/python/$python/g" /etc/init.d/daisy
chmod 0755 /etc/init.d/daisy

# Add daisy command
echo "Installing daisy command..."
cp -rf python/daisy.sh /usr/bin/daisy
sed -i "s/python/$python/g" /usr/bin/daisy
chmod 0755 /usr/bin/daisy

# Add daisy-passwd command
echo "Installing daisy-passwd command..."
cp -rf python/daisy-passwd.py /usr/bin/daisy-passwd
