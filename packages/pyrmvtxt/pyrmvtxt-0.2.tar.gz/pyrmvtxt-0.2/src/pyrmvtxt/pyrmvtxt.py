...
# In[] Split Image into smaller part and do Text detect
def additional_cropping_img(img, x1, x2, y1, y2, num, show=None):
    cropped_image = img[y1:y2, x1:x2]
    return cropped_image
def Split_Img(original_image_path):
...
# In[] Main function to detect and remove text in the  images
def removetxt(Images):
    original_img =keras_ocr.tools.read(Images)
    splitted_img,img_size,splitted_img_name=Split_Img(Images)
    print("*****Process on file : ",Images)
    for i in range(len(splitted_img)):
        print("***Start processing splitting : ",splitted_img_name[i],"(",i+1,"/",len(splitted_img),")")
