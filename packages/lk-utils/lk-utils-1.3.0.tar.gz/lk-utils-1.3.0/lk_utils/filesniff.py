...
@Desc    : Get filepath in elegant way.
 Note: in this module getters' behaviors are somewhat different from `os.path`
 or `pathlib`, see below:
    ...
    3. `filesniff` promtes progressing speed at the expense of sacrificing path
       legality checking in some situations, it means `filesniff` would maybe
...
def get_dirname(dirpath: str) -> str:
    """ Input a dirpath, return the dirname.
    *We don't check the dirpath exists or not.*
    """
    return dirpath.strip('/').rsplit('/', 1)[-1]
