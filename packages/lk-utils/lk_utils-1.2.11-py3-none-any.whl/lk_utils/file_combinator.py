...
def combine_excel(ifiles, ofile='', module=True, one_sheet=True):
    ...
    ARGS:
        ifiles: 来源文件, 可以是多个, 以列表或 iterable 形式传入
        ofile: 目标输出文件的路径. 为空则表示在 idir 目录下生成一个 merged.xlsx
            文件. ofile 的后缀名只能是 '.xlsx' (当为 '.xls' 时, 会被强制转换)
    ...
    for fileno, exl in lk.enum(ifiles, 1):
        ...
        if one_sheet is False:
            ...
            符)或重名时会报错, 因此我们加一个 modulex 前缀避免重名, 再裁剪至最多
            31 个字符.
            """
