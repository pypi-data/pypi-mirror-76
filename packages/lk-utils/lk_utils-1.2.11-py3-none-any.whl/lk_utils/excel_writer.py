...
class ExcelWriter:
    ...
    def __init__(self, filepath: str, sheet_name='', **options):
        """
        :param filepath: A filepath endswith '.xlsx' ('.xls' is not supported).
        :param sheet_name: Union[str, None].
            str: If string is empty, will create a sheet with default name --
             'sheet 1'.
            None: `None` is a special token, it tells ExcelWriter NOT create
    ...
    def merging_visual(self, rows, to_left='<', to_up='^',
        ...
        for uid, pos in merging_map.items():
            cell = uid_2_cell[uid]
            if len(pos) == 1:
                self.sheet.write(offset[0] + pos[0], offset[1] + pos[1], cell)
            else:
