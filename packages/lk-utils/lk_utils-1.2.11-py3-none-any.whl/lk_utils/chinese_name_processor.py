...
class Lexicon:
    ...
    def get(self, data_type, data_spec, data_fmt):
        # e.g. lexicon.get('pinyin', 'standard', 'trie')
        return self.res[data_type][data_spec][data_fmt]
...
class PinyinProcessor:
    ...
    def _init_tries(self, wade_mode):
        self.trie = Trie()
        if wade_mode:
    ...
    def _init_pinyin_patterns(self, wade_mode):
        """
        读取中国姓氏拼音列表
        """
    ...
    def is_chinese(self, name: str) -> bool:
        ...
        ARGS:
            name: 您可以传整个姓名, 也可以只传姓氏. 姓名只支持 'lastname,
                firstname' 格式的姓名. 当传入姓名时, 判断可能会更加准确. 例如,
                如果只传入 "jan", 那么在 self.wade_mode is True 的情况下, 会判断
                为 True; 而传入 "jan, edward" 和 "jan, ping xin", 则会分别判断为
                False 和 True.
        ...
        判断的, 后来发现有些罕见的姓, 虽然符合拼音或韦氏拼音的构造规则, 但由于不
        在 self.pinyin_patterns_4_name 范围内, 所以会被误判为非中国人名. 为避免
        这种情况, 不再使用 self.pinyin_patterns_4_name 提供的 pinyin 和 wade, 改
        用 self.pinyin_patterns 的 'pinyin+wade'.
        """
        if self.wade_mode:
    ...
    def is_pinyin(self, name, strict=False):
        ...
        NOTE:
            ...
            2. 建议 name 只传入单词
            2. 可能存在误识别. 例如 "sheer" 会被认作 ["she", "er"] -> True. 您可
               以通过传递 strict=True 参数来关闭分词模式
            3. 对于中英混合的情况, 会返回 False. 例如 "zhen cool" -> False
        """
        ...
        ARGS:
            ...
            trie_mode: str. 'postrie'/'trie'
                ...
                'trie': 正向最大匹配切词法. 例如:
                    ...
                    'daniel' -> ['dan', 'i', 'e', 'l']
        """
        self.obsv_mode = 'trie'  # obsv: observer  # DEL: no usage
