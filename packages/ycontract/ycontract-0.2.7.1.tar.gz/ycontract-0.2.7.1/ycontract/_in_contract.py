...
from .ycontract import DEFAULT_CONTRACT_STATE, ContractError, ContractState


class InContractException(Exception):
    ...


def new_in_contract_exception(
        f: Callable, f_args: list, f_kw: dict, cond_f: Callable[..., bool],
                    ...
                    for cond in conds:
                        if callable(cond):
                            if not in_contract_callable_one(cond, f, contract_tag, contract_state,
                                                            *args, **kw):
                                raise new_in_contract_exception(
                                    f, list(args), kw, cond, contract_tag)
                        else:
                            for varinfo, cond_info in cond.items():
                                ...
                                if callable(cond_info):
                                    ...
                                    cond_fs.extend(cond_info)

                                for cond_f in cond_fs:
                                    ex = get_contract_exception_from_dict(
                                        cond_f, f, varinfo, contract_tag, contract_state, *args,
                                        **kw)
                                    if ex is not None:
                                        raise ex

                    for varinfo, cond_info in cond_opts.items():
                        cond_fs = []
                        if callable(cond_info):
                            cond_fs = [cond_info]
                        else:
