...
if len(sys.argv) != 6:
    print("Computes the DER for one reference/hypothesis pair and a previous partial mapping and output the new mapping file.\nUsage:\n  %s ref.mdtm ref.uem hyp.mdtm input-map.txt output-map.txt\n" % sys.argv[0])
    sys.exit(0)
...
ref = load_mdtm(sys.argv[1])
uem = load_uem (sys.argv[2])
hyp = load_mdtm(sys.argv[3])
omap = load_map(sys.argv[4])

(der, time, map) = compute_der(ref, hyp, uem, omap, 0.250)
print("DER: %6.2f%%  Time: %.2fs" % (der, time))
