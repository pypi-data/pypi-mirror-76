...
subroutine differencemap(Fo,Fc,bin_idx,res_arr,smax,mode,nbin,nx,ny,nz, &
  ...
  if(debug) print*,'ibin  resol  scale_d  sqrt(Io/Ic)  bincount'
  do ibin=0, nbin-1
  ...
  do i=xyzmin(1), xyzmax(1)
     do j=xyzmin(2), xyzmax(2)
        do k=xyzmin(3), xyzmax(3)
           ...
           diffmap(i,j,k,0) = scale_d(bin_idx(i,j,k)) * Fc(i,j,k) - Fo(i,j,k)
           diffmap(i,j,k,1) = Fo(i,j,k) - scale_d(bin_idx(i,j,k)) * Fc(i,j,k)
  ...
  do i=xyzmin(1), xyzmax(1)
     do j=xyzmin(2), xyzmax(2)
        do k=xyzmin(3), xyzmax(3)
           if(bin_idx(i,j,k) < 0 .or. bin_idx(i,j,k) > nbin-1) cycle
           if(bin_idx(i,j,k) > cbin) cycle
           diffmap(i,j,k,0) = (Ec(i,j,k) - Eo(i,j,k)) * fsc(bin_idx(i,j,k))
           diffmap(i,j,k,1) = (Eo(i,j,k) - Ec(i,j,k)) * fsc(bin_idx(i,j,k))
