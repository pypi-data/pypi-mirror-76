...
def fsc_any(map1name,map2name,mode=None):
    from emda import xmlclass
    if mode=='half':
        # call halfmap_fsc calculation
        prepare_refmac_data(hf1name=map1name, hf2name=map2name)
    else:
        # call twomap_fsc calculation
        xml = xmlclass.Xml()
...
def difference_map(maplist, masklist, smax, mode='norm'):
    ...
    if mode=='norm':
        ...
        write_mrc(list_maps[0]*msk1, 'diffmap_m2-m1_nrm.mrc',uc, origin)
        write_mrc(list_maps[1]*msk2, 'diffmap_m1-m2_nrm.mrc',uc, origin)
    ...
    if mode=='ampli':
        ...
        write_mrc(list_maps[0]*msk1, 'diffmap_m2-m1_amp.mrc',uc, origin)
        write_mrc(list_maps[1]*msk2, 'diffmap_m1-m2_amp.mrc',uc, origin)
        write_mrc(list_maps[2]*msk1, 'map1.mrc',uc, origin)
        write_mrc(list_maps[3]*msk2, 'map2.mrc',uc, origin)
...
def prepare_refmac_data(hf1name,hf2name,outfile='fullmap.mtz', \
    bfac=None,maskname=None, xmlobj=None):
    import numpy as np
