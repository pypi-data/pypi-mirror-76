                    ...
                    if len(existing_list) > 0:
                        if len(new_list) > 0:
                            ...
                            if not existing_list_basictyped or not new_list_basictyped:
                                ...
                                raise Exception("Lists contain nonbasic types, must manually simplify. Error on key '{}'".format(nk))

                            if type(existing_list[0]) == type(new_list[0]):
                                # Types match, and they're both basictyped, can combine
                                res[nk].extend(nv)
                            else:
                                raise Exception("Lists contain incompatible types, must manually simplify. Error on key '{}'".format(nk))
