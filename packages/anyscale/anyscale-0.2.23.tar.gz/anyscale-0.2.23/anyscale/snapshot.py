...
def restore_docker_snapshot(
    ...
    cluster_config.setdefault("initialization_commands", []).extend(restore_commands)
    return cluster_config


def restart_registry_command() -> str:
    return (
        "docker stop registry || true &&"
        f"docker run -d -p 5555:5000 --name registry {REGISTRY_IMAGE}  || docker start registry"
    )
