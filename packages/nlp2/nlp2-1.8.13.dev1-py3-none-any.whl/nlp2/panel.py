    ...
    def add_element(self, k, v, msg, default):
        if isinstance(v, list):
            ...
            while inputted not in [str(e) for e in v]:
                inputted = input(msg + ", input an item in the list " + str(v) + ", [default=" + default + "]: ")
            self.element_list.append(inputted)
        else:
            if isinstance(v, float) and 0 < v < 1:  # probability
                invalue = float(input(msg + " (between 0-1), [default=" + default + "]: ") or v)
            elif isinstance(v, float):  # number
                invalue = float(input(msg + " (float), [default=" + default + "]: ") or v)
            elif isinstance(v, int):  # number
                invalue = int(input(msg + " (number), [default=" + default + "]: ") or v)
            else:
                invalue = input(msg + ", [default=" + default + "]: ")
