...
class ChangeProcess(Module):
    ...
    def _detection_penalty(self, action):
        ...
        if (action == STOP) or (
            ...
            logging.info(
                "Result of np.isnan(change_point): {}".format(
                    np.isnan(self._change_time)
                )
            )
            ...
            if self._simulation_time == self._max_duration and not np.isnan(
                self._change_time
            ):
...
class Threshold(Policy):
    ...
    def _reparameterise(params, **kwargs):
        ...
        Returns:
            [np.ndarray of float64]: free decision parameter
        """
        return logit(params)
...
class Hyperplanes(Policy):
    def __init__(self, num_states=None, num_pre_change_states=None, planes=None):
        ...
        if planes is not None:
            ...
            print(
                "Hyperplanes constructor set params : {}".format(
                    self._free_params
                )
            )
