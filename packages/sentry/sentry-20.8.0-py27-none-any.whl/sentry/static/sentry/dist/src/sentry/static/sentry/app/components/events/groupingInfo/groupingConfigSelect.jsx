...
var GroupingConfigSelect = /** @class */ (function (_super) {
    ...
    GroupingConfigSelect.prototype.renderBody = function () {
        ...
        var options = configs
            .filter(function (config) { return !config.hidden || config.id === eventConfigId; })
            .map(function (_a) {
            var id = _a.id;
            return ({
                value: id,
                label: (<GroupingConfigItem isActive={id === eventConfigId}>{id}</GroupingConfigItem>),
            });
        });
        return (<DropdownAutoComplete value={configId} onSelect={onSelect} alignMenu="left" selectedItem={configId} items={options}>
        {function (_a) {
