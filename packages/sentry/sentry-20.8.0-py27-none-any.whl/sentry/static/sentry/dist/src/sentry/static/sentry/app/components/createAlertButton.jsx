...
function IncompatibleQueryAlert(_a) {
    var incompatibleQuery = _a.incompatibleQuery, eventView = _a.eventView, onClose = _a.onClose;
    ...
    return (<StyledAlert type="warning" icon={<IconInfo color="yellow400" size="sm"/>}>
      ...
      <StyledCloseButton icon={<IconClose color="yellow400" size="sm" isCircled/>} aria-label={t('Close')} size="zero" onClick={onClose} borderless/>
    </StyledAlert>);
}
function incompatibleYAxis(eventView) {
    ...
    var invalidParameter = !isNumericParameter && !__spread([''], yAxisConfig.fields).includes(column.function[1]);
...
function CreateAlertButton(_a) {
    ...
    var to = hasErrors
        ? undefined
        : {
            pathname: "/settings/" + organization.slug + "/projects/" + (project === null || project === void 0 ? void 0 : project.slug) + "/alerts/new/",
            query: __assign(__assign({}, eventView.generateQueryStringObject()), { createFromDiscover: true, referrer: referrer }),
        };
    var handleClick = function (event) {
        if (hasErrors) {
            event.preventDefault();
            onIncompatibleQuery(function (onAlertClose) { return (<IncompatibleQueryAlert incompatibleQuery={errors} eventView={eventView} onClose={onAlertClose}/>); }, errors);
            return;
        }
        onSuccess();
    };
