...
var ExceptionContent = function (_a) {
    ...
    var children = values.map(function (exc, excIdx) { return (<div key={excIdx} className="exception">
      ...
      <ExceptionStacktraceContent data={type === 'original' ? exc.stacktrace : exc.rawStacktrace || exc.stacktrace} stackView={stackView} stacktrace={exc.stacktrace} expandFirstFrame={excIdx === values.length - 1} platform={platform} newestFirst={newestFirst} event={event} chainedException={values.length > 1}/>
    </div>); });
    if (newestFirst) {
        children.reverse();
    }
