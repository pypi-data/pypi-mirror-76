...
Storage folder `gs://cloud-tpu-checkpoints/bert`. For example:
...
The default vocabulary has a number of unused tokens, `[unused0]`-`[unused993]`,
that can be used to mark phenomena in your input. If you're training a new model
that utilizes these tokens, it's recommended that you set the flag
`preserve_unused_tokens` to True so that they're preserved during
tokenization. This flag is set to False by default to preserve old
functionality, but this functionality will become standard in the future.
