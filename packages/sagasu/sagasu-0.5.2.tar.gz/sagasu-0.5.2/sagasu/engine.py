...
class SearchEngine:
    ...
    def load_indexed() -> IndexedResource:
        ...
        p = reduce(
            ...
            [
                p
                for p in Path(f"{SAGASU_WORKDIR}/indexed").iterdir()
                if p.suffix == ".pkl"
            ],
        )
    ...
    def reduce_indexing_stream(self, dump=True):
        def _reduce_indexed_resources(
            r1: IndexedResource, r2: IndexedResource
        ) -> IndexedResource:
            for r in r1.indexed:
                if r in r2.indexed:
        ...
        async def run_stream(
            _loop: asyncio.AbstractEventLoop, crawlers: List[Crawler]
            ...
            return await asyncio.gather(
                *[_run_indexing_stream(crawler) for crawler in crawlers]
            )

        loop = asyncio.get_event_loop()
        crawler_engine = CrawlerEngine(self.config.sources)
        indexed_resources = loop.run_until_complete(
            run_stream(loop, crawler_engine.crawlers)
        )
        ...
        if dump:
            mkdir_p(
                file_path := f"{SAGASU_WORKDIR}/indexed/{datetime.datetime.now(JST).strftime('%Y-%m-%d-%H')}.pkl"
            )
    ...
    def indexing_stream(self, crawler: Crawler) -> IndexedResource:
        ...
        jobs = [
            delayed(_indexing)(*resource_indexer_pair)
            for resource_indexer_pair in resource_indexer_pairs
        ]
...
class CrawlerEngine:
    def __init__(self, sources: List[SourceModel]):
        ...
        self.crawlers: List[Crawler] = [
            self._load_crawler(source) for source in sources
        ]
