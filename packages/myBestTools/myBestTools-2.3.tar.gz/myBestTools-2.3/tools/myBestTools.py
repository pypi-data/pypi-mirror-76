...
class Tool:
    ...
    def post_html(self, url, headers={}, data={}, json={}, proxy={}, timeout=5, debug=False):

        for i in range(5):
            try:
                res = self.session.post(url, data=data, json=json, headers=headers, timeout=timeout, proxies=proxy)
                return res
            except Exception as e:
                if debug:
                    logging.info(e)
