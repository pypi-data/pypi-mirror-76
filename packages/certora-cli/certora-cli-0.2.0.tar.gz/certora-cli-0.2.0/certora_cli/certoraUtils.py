...
def checkExecException(jsonResponse: Dict[str, Any], statusUrl: str) -> bool:
...
def pollOutput(url: str, statusUrl: str, nextIndex: int = 0, lim: int = 60) -> bool:
    has_output = True
    result = False
    ...
    while True:
        try:
            ...
            if not jsonResponse:  # Error parsing json
                ...
                break
            if not checkExecException(jsonResponse, statusUrl):  # look for execution exceptions
                break
            try:
            ...
            output_l = len(allOutput)
            if nextIndex < output_l:  # there is a new output
                has_output = True
                for i in range(nextIndex, output_l):
...
def lookForIndex(indexPath: str, statusUrl: str) -> None:
    try:
        ...
        if (r.status_code == requests.codes.ok):
            ...
            print(ErrorMsgPrefix, "Failed to fetch index.html.")
            print("Please, contact Certora on https://www.certora.com")
            return
        else:
            errorMsg = "index.html was not created."
            printErrorMessage(errorMsg, statusUrl)
...
def cliVerify(args: Dict[str, str], compareToExpected: bool = True) -> bool:
    ...
    if "settings" in args:
        ...
        # uncommit these lines on next Vaas deploy
    ...
    try:
        if pollOutput(outputUrl, statusUrl):
    ...
    if compareToExpected:
        ...
        return checkResultsFromWeb(outputPath)
    return True
