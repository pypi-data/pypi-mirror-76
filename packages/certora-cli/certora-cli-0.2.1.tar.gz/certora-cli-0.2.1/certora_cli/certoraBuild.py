...
class CertoraBuildGenerator:
    def __init__(self, input_config: InputConfig) -> None:
        ...
        self.asts = {}  # type: Dict[str, Dict[int, Any]]
    ...
    def collect_funcs(self, contract: str, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        funcs = []
        ...
        for funcstr, hash in hashes.items():
    ...
    def collect_asts(self, contract_sources: Dict[str, Dict[str, Any]]) -> None:
        for c in contract_sources:
            container = {}  # type: Dict[int, Any]
            self.asts[c] = container
            queue = [contract_sources[c]]
            while queue:
                pop = queue.pop(0)
    ...
    @staticmethod
    def get_node_from_asts(asts: Dict[str, Dict[int, Any]], node_id: int) -> Any:
        for contract_file in asts:
            node = asts.get(contract_file, {}).get(node_id)
            if node is not None:
    ...
    def collect_immutables(self,
        ...
        self.collect_asts(contract_sources)
        for astnode_id in immutableReferences:
            astnode = self.get_node_from_asts(self.asts, int(astnode_id))
        ...
        self.asts.clear()  # Remove all asts as they are no longer needed
        return out
    ...
    def collect_for_file(self, file: str, file_index: int) -> SDC:
        ...
        contracts = {file_abs_path: [c for c in data["contracts"][file_abs_path]]}
        contracts_with_libraries = {}
        # Need to add all library dependencies that are in a different file:
        for contract_file, contract_list in contracts.items():
            ...
            for contract_name in contract_list:
                ...
                for linkRef in linkRefs:
                    contracts_with_libraries[linkRef] = [c for c in data["contracts"][linkRef]]
    ...
    def get_contract_in_sdc(self,
                            ...
                            data: Dict[str, Any],
                            fetched_source: str,
                            primary_contract: str) -> ContractInSDC:
        ...
        funcs = self.collect_funcs(contract_name, contract_data)
        ...
        immutables = self.collect_immutables(contract_data, data["sources"], contract_file)
