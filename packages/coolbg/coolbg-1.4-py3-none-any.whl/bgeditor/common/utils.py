...
def upload_file(path):
    public_folder_id = "1bHFdiVaYs14AxHe_ei-w9AOmOJh1Axk_"
    file_name = os.path.basename(path)
    return Client("/u02/drive_config/music_config/coca_idrive.json", "upload", path, "").upload_file(file_name,path, public_folder_id)
def download_gdrive(id,path):
    Client("/u02/drive_config/music_config/coca_idrive.json", "download", path, "").download_file(id,path)
