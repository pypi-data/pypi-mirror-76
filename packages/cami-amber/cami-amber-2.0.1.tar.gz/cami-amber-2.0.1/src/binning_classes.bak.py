...
class GenomeQuery(Query):
    ...
    def compute_metrics(self):
        ...
        gs_df = self.gold_standard_df.drop_duplicates(['SEQUENCEID', 'BINID'])
        gs_df = gs_df[['SEQUENCEID', 'BINID', 'LENGTH']].rename(columns={'LENGTH': 'seq_length', 'BINID': 'genome_id'})

        query_df = self.df.drop_duplicates(['SEQUENCEID', 'BINID'])
        ...
        print(query_w_length_mult_seqs)
        print(query_w_length_mult_seqs[query_w_length_mult_seqs['SEQUENCEID'] == 'k141_104699'])
        # exit()
        ...
        if query_w_length.shape[0] < query_w_length_mult_seqs.shape[0]:
            ...
            print(query_w_length_mult_seqs)
            # print(query_w_length_mult_seqs.head(20))
            # exit()
            ...
            print("most_abundant_genome_df")
            print(most_abundant_genome_df)
            # exit()

            matching_genomes_df = pd.merge(query_w_length_mult_seqs, most_abundant_genome_df, on=['BINID', 'genome_id']).set_index('index')
            print("matching_genomes_df")
            print(matching_genomes_df)
            print(matching_genomes_df[matching_genomes_df['SEQUENCEID'] == 'k141_104699'])
            # exit()
            ...
            print("query_w_length")
            print(query_w_length)
            print(query_w_length[query_w_length['BINID'] == 'bin.100.fa'])
            print(query_w_length[query_w_length['SEQUENCEID'] == 'k141_1001247'])
            print(query_w_length[query_w_length['SEQUENCEID'] == 'k141_104699'])
            print(query_w_length[query_w_length['SEQUENCEID'] == 'k141_110734'])
            print('unique genomes (query_w_length): ', query_w_length['genome_id'].nunique())
            print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')

            print(gs_df)
            print(gs_df.shape)
            # gs_df.to_csv('/home/fmeyer/tmp/amber_genome_megahit1/gs.tsv', sep='\t')
            print(gs_df[gs_df['SEQUENCEID'] == 'k141_1001247'])
            print(gs_df[gs_df['SEQUENCEID'] == 'k141_104699'])
            print(gs_df[gs_df['SEQUENCEID'] == 'k141_110734'])

            matching_genomes_df = pd.merge(gs_df, query_w_length[['SEQUENCEID', 'genome_id']], on=['SEQUENCEID', 'genome_id'], how='inner', indicator=True)
            print("matching_genomes_df")
            print(matching_genomes_df)
            print(matching_genomes_df.shape)

            print(matching_genomes_df[matching_genomes_df.duplicated()])
            print(matching_genomes_df[matching_genomes_df['SEQUENCEID'] == 'k141_110734'])
            # exit()

            # x = matching_genomes_df[matching_genomes_df['_merge'] != 'both']
            # print(x)
            # exit()

            # matching_genomes_df.to_csv('/home/fmeyer/tmp/amber_genome_megahit1/matching_genomes.tsv', sep='\t')
            matching_genomes_df = matching_genomes_df[['SEQUENCEID', 'genome_id', 'seq_length']].drop_duplicates(['SEQUENCEID', 'genome_id'])
            print("matching_genomes_df after drop_duplicates")
            print(matching_genomes_df)
            print(matching_genomes_df[matching_genomes_df['SEQUENCEID'] == 'k141_1001247'])
            print(matching_genomes_df[matching_genomes_df['SEQUENCEID'] == 'k141_104699'])



            # difference_df = gs_df.drop()
            condition = gs_df_no_dups['SEQUENCEID'].isin(matching_genomes_df['SEQUENCEID'])
            difference_df = gs_df_no_dups[~condition]
            print("difference_df")
            print(difference_df)

            # gs_df = pd.merge(difference_df, matching_genomes_df, on='SEQUENCEID', how='outer')
            gs_df = pd.concat([difference_df, matching_genomes_df])
            print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            print(gs_df)
            print(gs_df[gs_df['SEQUENCEID'] == 'k141_1001247'])
            print(gs_df[gs_df['SEQUENCEID'] == 'k141_104699'])
            print('unique genomes (gs): ', gs_df['genome_id'].nunique())

            # gs_df = pd.merge(gs_df_no_dups, matching_genomes_df, on='SEQUENCEID', how='left')
        ...
        # print(confusion_df)
        # confusion_df.to_csv('/home/fmeyer/tmp/amber_genome_megahit1/confusion_df.tsv', sep='\t')
        # exit()
        ...
        # print(recall_df)
        # recall_df.to_csv('/home/fmeyer/tmp/amber_genome_megahit1/recall.tsv', sep='\t')
        # exit()
