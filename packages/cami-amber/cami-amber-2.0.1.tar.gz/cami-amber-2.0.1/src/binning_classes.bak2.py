...
class GenomeQuery(Query):
    ...
    def plot_precision_vs_bin_size(self):
        ...
        self.precision_df.to_csv(os.path.join(self.options.output_dir, 'genome', self.label, 'precision_df_' + self.sample_id + '.tsv'), sep='\t')
    ...
    def plot_recall_vs_genome_size(self):
        fig, axs = plt.subplots(figsize=(5, 4.5))
        self.recall_df.to_csv(os.path.join(self.options.output_dir, 'genome', self.label, 'recall_df_' + self.sample_id + '.tsv'), sep='\t')
...
class TaxonomicQuery(Query):
    ...
    def compute_tp_fp_fn(self, rank):
        ...
        # print(rank)
        # list1 = tp_fp_fn_df.index[tp_fp_fn_df.loc[:, 'total_length'] > 0]
        # print(tp_fp_fn_df.loc[list1]['total_length'].mean())
        ...
        # list1 = tp_fp_fn_df.index[(tp_fp_fn_df.loc[:, 'total_length'] > 0) & tp_fp_fn_df['precision_bp'].isna()]
        # print(tp_fp_fn_df.loc[list1]['total_length'].mean())
    ...
    def compute_metrics(self):
        ...
        ranks_df = self.__tp_fp_fn_df.groupby('rank')
        for rank in load_ncbi_taxinfo.RANKS[0:-1]:
            # print(rank)
            tp_fp_fn_df = ranks_df.get_group(rank)
            # print(tp_fp_fn_df[['total_length', 'precision_bp']])
            list1 = tp_fp_fn_df.index[tp_fp_fn_df.loc[:, 'total_length'] > 0]
            print('{}\t{}'.format(rank, tp_fp_fn_df.loc[list1]['total_length'].mean()))
