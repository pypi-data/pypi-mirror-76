...
def plot_by_genome_coverage(pd_bins, pd_target_column, available_tools, output_dir):
    colors_list = create_colors_list()[1:]
    ...
    axs.set_xlim([-0.05, pd_tool['genome_coverage'].max() + 0.05])
    ...
    plt.xlabel('Average genome coverage', fontsize=14)
    ...
    fig.savefig(os.path.join(output_dir, 'genome', file_name + '.pdf'), dpi=100, format='pdf', bbox_inches='tight')
    plt.close(fig)
...
def plot_summary(color_indices, df_results, labels, output_dir, rank, plot_type, file_name, xlabel, ylabel):
    ...
    tools = ['Gold standard'] + [tool for tool in labels if tool in available_tools]
    ...
    plt.ylabel(ylabel, fontsize=14)
    plt.tight_layout()
    fig.savefig(os.path.join(output_dir, binning_type, file_name + '_no_legend.pdf'), dpi=100, format='pdf', bbox_inches='tight')
