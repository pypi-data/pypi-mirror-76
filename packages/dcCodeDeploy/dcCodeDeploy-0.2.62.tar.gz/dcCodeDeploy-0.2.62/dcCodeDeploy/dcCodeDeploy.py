...
@task
def build2(javaEnv, dirs, prodSecrets="False",shard="",buildApi="True", buildWorker="True"):
	...
	if buildApi in TRUTH_VALUES:
        ...
        if prodSecrets in TRUTH_VALUES:
            local('aws s3 cp s3://topopps-prod-x/keyset-%s-%s.json deploy/api/config/keyset.json' % (javaEnv, shard))
            local('aws s3 cp s3://topopps-prod-x/application-%s-%s.yml deploy/api/config/application-%s.yml' % (javaEnv, shard, javaEnv))
        else:
	...
	if buildWorker in TRUTH_VALUES:
        ...
        if prodSecrets in TRUTH_VALUES:
            local('aws s3 cp s3://topopps-prod-x/keyset-%s-%s.json deploy/worker/config/keyset.json' % (javaEnv, shard))
    	    local('aws s3 cp s3://topopps-prod-x/application-%s-%s.yml deploy/worker/config/application-%s.yml' % (javaEnv, shard, javaEnv))
    	    local('aws s3 cp s3://topopps-prod-x/application-%s-%s-worker.yml deploy/worker/config/application-%s-worker.yml' % (javaEnv, shard, javaEnv))
        else:
