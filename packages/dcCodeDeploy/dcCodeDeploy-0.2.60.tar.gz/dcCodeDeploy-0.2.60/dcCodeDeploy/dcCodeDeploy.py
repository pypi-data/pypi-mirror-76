...
@task
def build2(javaEnv, dirs, prodSecrets="False",buildApi="True", buildWorker="True"):
	...
	if buildApi in TRUTH_VALUES:
        ...
        if prodSecrets in TRUTH_VALUES:
            local('aws s3 cp s3://topopps-prod-x/keyset-%s.json deploy/api/config/keyset.json' % javaEnv)
            local('aws s3 cp s3://topopps-prod-x/application-%s.yml deploy/api/config/' % javaEnv)
        else:
	...
	if buildWorker in TRUTH_VALUES:
        ...
        if prodSecrets in TRUTH_VALUES:
            local('aws s3 cp s3://topopps-prod-x/keyset-%s.json deploy/worker/config/keyset.json' % javaEnv)
    	    local('aws s3 cp s3://topopps-prod-x/application-%s.yml deploy/worker/config/' % javaEnv)
    	    local('aws s3 cp s3://topopps-prod-x/application-%s-worker.yml deploy/worker/config/' % javaEnv)
        else:
