...
@click.option("--yes", "-y", default=False, is_flag=True, required=False, help="Force yes")
@click.pass_obj
def kitchen_revert(backend: Backend, kitchen: str, yes: bool) -> None:
    """
    ...
    try:
        ...
        message = f"{get_datetime()} - Reverting kitchen {use_kitchen} to previous state. Local files will not change."
        message += os.linesep + "After this operation, run recipe-get -o in the related recipes."

        click.secho(message, fg="green")

        DKCloudCommandRunner.revert_kitchen(backend.dki, use_kitchen)
