...
class Chord(object):
    ...
    def scale_transpose(self, scale_obj, steps):
        return self.with_notes([ x.scale_transpose(scale_obj, steps) for x in self.notes ])


    def transpose(self, steps=None, semitones=None, octaves=None):
        return self.with_notes([ note.transpose(steps=steps, octaves=octaves, semitones=semitones) for note in self.notes ])
