...
class conductorRender(om.MPxNode):
    ...
    def detect_plugins(cls, fn_node):
        """Detect ONE renderer only. 
        """
        renderer_version = software.detect_mtoa() or software.detect_rfm(
        ) or software.detect_vray() or software.detect_redshift()
        if renderer_version:
            fn_node.findPlug(cls.aPluginSoftware, True).elementByLogicalIndex(
                0).setString(renderer_version)
