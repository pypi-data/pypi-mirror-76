...
class ValidateSelectedRenderer(Validator):
    def run(self, layername):
        ...
        if required_package not in [p.split(" ")[0] for p in pm.PyNode("conductorRender1").attr("pluginSoftware").get()]:
            self.add_warning(
...
def run(node):
    ...
    if errors:
        ...
        msg = "Some issues prevented your submission. See the script editor for details."
        raise ValidationError(msg)
    if notices or warnings:
        dialog_result = pm.layoutDialog(
            ui=pm.Callback(result_window, warnings, notices), title="Submission notices")
...
def result_window(warnings, notices):
    form = pm.setParent(q=True)
    text = pm.text(
        label="Please read the notices before you continue as as some issues may cause undesired results!")

    b1 = pm.button(label="Cancel", command=pm.Callback(
        pm.layoutDialog, dismiss="abort"))
    b2 = pm.button(label="Submit", command=pm.Callback(
