...
def get_new_model_version(current_model_version):
    new_model_version = current_model_version.split('.')
...
def create_saved_model_folder(model_name):
    print('mudança sutil no utils')
    last_model_path = ''
    current_model_version = json.loads(read_file(os.getcwd() + '/config.txt'))['version']
    new_model_version = get_new_model_version(current_model_version)

    model_path = os.path.join('./src/nkia/ml/saved_model/' + model_name + '/')
    os.makedirs(model_path + new_model_version)

    if current_model_version in os.listdir(model_path):
        last_model_path = model_path + current_model_version        
