...
class நிரல்மொழித்தகவல்கள்(object):
    ...
    def தொகுத்தல்(தன்):
        ...
        with open(f"{தன்.கோப்புறை}/_தகவல்கள்.py", 'w', encoding='utf8') as கோ:
            உரை = 'தகவல்கள்="""' + json.dumps(
                மொழிபெயர்ப்பு_அகராதிகள், ensure_ascii=False, indent=2
            ).replace('\\', '\\\\') + '"""'
            கோ.write(உரை)

        with open(f"{தன்.கோப்புறை}/_மொழிபெயர்ப்புகள்.py", 'w', encoding='utf8') as கோ:
            உரை = f'மொழிபெயர்ப்புகள்="""{json.dumps(தன்._தகவல்கள், ensure_ascii=False, indent=2)}"""'
            கோ.write(உரை)

    def _தகவல்கள்_படி(தன்):
        try:
            from .இலக்கணங்கள்._தகவல்கள் import தகவல்கள்
            from .இலக்கணங்கள்._மொழிபெயர்ப்புகள் import மொழிபெயர்ப்புகள்
            தகவல்_அகராதி = json.loads(தகவல்கள்)
            மொழிபெயர்ப்பு_அகராதி = json.loads(மொழிபெயர்ப்புகள்)
    ...
    def _புதுப்பிப்பு(தன், அக, பு):
        for சா, ம in பு.items():
            if isinstance(ம, dict):
                அக[சா] = தன்._புதுப்பிப்பு(அக.get(சா, {}), ம)
            else:
                அக[சா] = ம
        return அக
