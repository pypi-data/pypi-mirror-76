...
class Interval(Loop):
    def __init__(self, seconds, pre=False):
        super(Interval, self).__init__(seconds=seconds, pre=pre)
    ...
    def _set(self, **kwargs):
        super(Interval, self)._set(**kwargs)
    ...
    def json(self):
        return {**super(Interval, self).json(), "next": self.next}


class IntervalIndex(Interval):
    def loop(self):
        for _ in super(IntervalIndex, self).loop():
            yield self.n
