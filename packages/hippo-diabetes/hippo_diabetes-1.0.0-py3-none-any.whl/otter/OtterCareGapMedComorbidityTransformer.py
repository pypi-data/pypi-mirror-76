...
from test_OtterCareGapComorbidityTransformer import test_OtterCareGapComorbidityTransformer
os.chdir(cwd)

class OtterCareGapMedComorbidityTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        returnX =  pd.DataFrame(zip(X[self.comorbidity],
                            ...
                            columns=[self.comorbidity, self.careGapName])

        #add on insure med columns
        returnX[self.comMedCol] = X[self.comMedCol]
        #run unit test, stops code if anything is wrong
        test_OtterCareGapComorbidityTransformer().test_transformer(returnX)
