...
class OtterCareGapMedAdherenceAggregatePipeline:
    """Aggregate med adherence care gaps
    ...
    def addPipe(self,X):
        ...
        for key in self.modelParamsDict.keys():
            ...
            X = pd.concat([X, OtterCareGapMedAdherencePipeline(careGapNameValue=key,
                                                                    comMedCol='insure_med',
                                                                    drugCol=self.modelParamsDict[key][0],
                                                                    drugPDCCol=self.modelParamsDict[key][1],
                                                                    comMedOutcomeValue='neither').addPipe().transform(X)], axis = 1)
