...
class OtterCareGapMedMedOpt4thLinePipeline:
    ...
    :type a1cCriteriaValue: pd.Interval()
    :param includeComorbCol: columns of any condition comorbidity to include
    :type includeComorbCol: string
    :param excludeComorbCol: columns of any condition comorbidity to exclude
    :type excludeComorbCol:string
    ...
    def __init__(self, careGapNameValue, a1cValueCol, a1cCriteriaValue, includeComorbCol, excludeComorbCol, 
                requiredAntiDiabeticDrugCol,requiredAntiDiabeticDrugPDCCol, countOfAnyRequiredDrugValue, comMedOutcomeValue,extraDrugCol,extraDrugCountCol, drugListCol=None, drugPDCListCol=None,comMedCol=None, selectorList= None,drugLongerThan12moList = None):
        """constructor method
        """
        ...
        self.includeComorbCol = includeComorbCol
        self.excludeComorbCol = excludeComorbCol
        ...
        if drugLongerThan12moList == None:
            drugLongerThan12moList = ['rx_sulfonylureas_longer_12_month', 'rx_biguanides_longer_12_month','rx_thiazolidinediones_longer_12_month','rx_dpp_4_inhibitors_longer_12_month','rx_meglitinide_analogues_longer_12_month',
                                      'rx_sglt2_inhibitors_longer_12_month','rx_glp1_agonist_longer_12_month', 'rx_alpha_glucosidase_inhibitor_longer_12_month','rx_insulin_longer_12_month','rx_statins_longer_12_month',
                                     'rx_glipizide_longer_12_month']
        self.drugPDCListCol = drugPDCListCol
        self.selectorList = selectorList
        self.drugLongerThan12moList = drugLongerThan12moList

    # apply pipe function
    def addPipe(self):

        #conduct pipeline
        medicareMedOptPipeline = Pipeline(steps=[
            ('OtterFeatureSelector', OtterFeatureSelector(featureNames=[self.comMedCol]+self.drugListCol + self.drugPDCListCol+self.drugLongerThan12moList+ self.selectorList)),
            ('OtterCareGapMedMedOptTransformer', OtterCareGapMedMedOpt4thLineTransformer(careGapName=self.careGapNameValue,
                                                                                  comMedCol=self.comMedCol,
                                                                                  a1cValue = self.a1cValueCol,
                                                                                  a1cCriteria = self.a1cCriteriaValue,
                                                                                  includeComorb = self.includeComorbCol,
                                                                                  excludeComorb = self.excludeComorbCol,
