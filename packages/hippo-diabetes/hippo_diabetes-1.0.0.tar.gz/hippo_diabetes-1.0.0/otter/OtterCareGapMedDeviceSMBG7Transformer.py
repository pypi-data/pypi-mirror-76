...
class OtterCareGapMedDeviceSMBG7Transformer(BaseEstimator, TransformerMixin):
    ...
    def deviceFilter2(self, hypoglycemicDx, drugInsulin12,deviceTest,SUDx12, a1cThresh):
        ...
        NVL = UtilAPI().NVL
        # among those who are eligible
        if NVL(hypoglycemicDx,0) == 0 and NVL(drugInsulin12,0) == 0 and NVL(SUDx12,0) == 1 and NVL(a1cThresh,0) < 7:
            # if having 1+ claims of testing, then closed
            if np.nansum(deviceTest) >= 1:
                return int(1)
