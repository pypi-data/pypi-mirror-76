...
from .exceptions import NotAGitCommandException
...
def execute_git_command(path, command):
    if not os.path.isdir(path):
        raise FileNotFoundError("This path ({}) does not exist".format(path))
        return []
    bash_command = "git -C " + str(path) + " " + command
    try:
        output = subprocess.run(split(bash_command), stdout=subprocess.PIPE)
        result = output.stdout.decode("utf-8").splitlines()
        return result
    except subprocess.CalledProcessError:
        raise NotAGitCommandException("Not a git command")
...
def get_git_objects(path):
    objects = [
        line.split()
        for line in execute_git_command(
...
def get_git_references(path):
    ...
    local_branches = {
        ref[2][len(lbr) :]: ref[0] for ref in references if ref[2].startswith(lbr)
    }
    remote_branches = {
        ref[2][len(rbr) :]: ref[0]
