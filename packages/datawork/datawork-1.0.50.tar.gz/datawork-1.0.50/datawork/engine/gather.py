...
class DragonGather(TaskScheduler):
    ...
    async def gather_data(self, ipt, ids, *args, **kwargs):
        ...
        if not self.rethinkdb_origin.get(ids):
            ...
            try:
                await wait_for(rc.async_connect(), timeout=10)
        ...
        if body or origin or destiny:
            ...
            try:
                ...
                rc.logerror(
                    f'''IDEX = {idex}, Error en la obtención de datos para estación {code} en {ex}'''
                    ...
                    try:
                        ...
                        try:
                            await wait_for(
                               ...
                               ConnectionAbortedError) as conn_error:
                           idex=my_random_string(uin)
                           kwargs["origin_exception"]=f'''PD_TO3_-1 + {idex} +
            {code} + {conn_error}, ConnectionClosed rc_enu connection....'''
                           log.exception(kwargs["origin_exception"])
                           self.rethinkdb_origin[ids]=False
                           bprint(kwargs)
                           rc.close()
