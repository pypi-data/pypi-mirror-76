...
class DragonGather(TaskScheduler):
    ...
    def add_sta_instance_origin(self, ids, loop):
        ...
        db_datos = dict(host='localhost',
                        ...
                        io_loop=loop,
                        log_path=str(Path(self.log_path)/"source_rdb"),
                        env='gather_%s' % code)
        name = 'RethinkDB'
    ...
    def add_sta_instance_destiny(self, ids, loop):
        ...
        db_datos_enu = dict(host='localhost',
                            ...
                            dbname='enu_data',
                            io_loop=loop,
                            log_path=str(Path(self.log_path)/"enu_rdb"),
                            env='enu_%s' % code)
    ...
    def add_process_instance(self, ids):
        ...
        kwargs['log_path'] = str(Path(self.log_path)/'geo_json_data')
    ...
    async def gather_data(self, ipt, ids, *args, **kwargs):
        ...
        if body or origin or destiny:
            ...
            try:
                ...
                cursor = await future
                gprint("____")
                bprint(f"Cursor len {len(cursor)}")
                gprint("____")
            ...
            for c in cursor:
                if check_gsof(c):
                    try:
                        ...
                        bprint(f"Data {to_db}")
                        try:
                            await rc_enu.save_data(table_name, to_db)
    ...
    def set_init_args_kwargs(self, ipt):
        ...
        log = LogFile("Engine@Datawork", "CORE_%s" %
                      ...
                      path=str(Path(self.log_path)/"engine"))
