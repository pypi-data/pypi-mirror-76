...
//if (dev->dev_index == t_Capture) printf("Read %d frames\n", frames);
	if ( ! bytes)
		return 0;
		...
		complex double * cSamples, int report_latency, double volume)
{
	...
	if (IDirectSoundBuffer8_GetCurrentPosition(ptBuf, &playPos, &writePos) != DS_OK) {
		if (quisk_sound_state.verbose_sound)
			printf ("Bad GetCurrentPosition for %s\n", dev->stream_description);
	...
	case 1:	// Normal run state
		...
		// Check if play buffer is too full.
			...
			dev->started = 2;
		        ...
		        nSamples--;
#if DEBUG_IO
		        printf("play_alsa %s: Remove a sample\n", playdev->stream_description);
#endif
	        }
		        ...
		        nSamples++;
#if DEBUG_IO
		        printf ("play_alsa %s: Add a sample\n", playdev->stream_description);
#endif
	        }
		break;
	case 2:	 // Buffer is too full; wait for it to drain
		if (buffer_fill <= 0.5) {
			dev->started = 1;
			if (quisk_sound_state.verbose_sound)
				printf("Resume adding samples for %s\n", dev->stream_description);
		}
		else {
			nSamples = 0;
		}
	...
	if (IDirectSoundBuffer8_Lock(ptBuf, dev->dataPos, bytes, &pt1, &n1, &pt2, &n2, 0) != DS_OK) {
		if (quisk_sound_state.verbose_sound)
			printf ("DirectX play lock error for %s\n", dev->stream_description);
		quisk_sound_state.write_error++;
		dev->dev_error++;
		return;
	}
	pass = 0;
	n = 0;
//if (dev->dev_index == t_Playback) printf("Buffer fill %.3f%% bytes %d for %s\n", buffer_fill * 100, bytes, dev->stream_description);
	switch (dev->sound_format) {
	case Int16:
