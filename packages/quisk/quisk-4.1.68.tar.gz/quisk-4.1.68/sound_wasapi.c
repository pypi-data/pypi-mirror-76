...
struct dev_data_t {
	...
	HANDLE hEvent;
	int playbuf_read_reset;		// playbuf is used for the play device callback
...
static LPWSTR to_pwsz(const char *str) {
...
static DWORD WINAPI playdevice_callback(LPVOID lpParameter)	// Called by a special thread.
{  // Read samples from the Quisk play_buffer and write them to the sound card.
	...
	while (1) {
		...
		if (quisk_play_state < RECEIVE) {
			...
			//printf("%s:	 playdevice: write silence\n", dev->stream_description);
			continue;
		}
		iRead = DD->playbuf_iRead;
		buffer_fill = DD->playbuf_iWrite - iRead;       // must be atomic
		if (buffer_fill < 0)
			buffer_fill += DD->playbuf_nFrames;
		if (DD->playbuf_read_reset) {
			if (buffer_fill >= DD->playbuf_nFrames / 2) {
				DD->playbuf_read_reset = 0;
				if (quisk_sound_state.verbose_sound)
					printf("%s:  playdevice: buffer underflow recovery\n", dev->stream_description);
			}
			else {
				IAudioRenderClient_ReleaseBuffer(DD->pRenderClient, DD->bufferSizeFrames, AUDCLNT_BUFFERFLAGS_SILENT);
				//printf("%s:	 playdevice: write silence\n", dev->stream_description);
		...
		else if (buffer_fill <= DD->bufferSizeFrames) {
			DD->playbuf_read_reset = 1;
			dev->dev_underrun++;
			if (quisk_sound_state.verbose_sound)
				printf("%s:  playdevice: buffer underflow\n", dev->stream_description);
			IAudioRenderClient_ReleaseBuffer(DD->pRenderClient, DD->bufferSizeFrames, AUDCLNT_BUFFERFLAGS_SILENT);
			//printf("%s:	 playdevice: write silence\n", dev->stream_description);
			continue;
		}
		...
		DD->playbuf_iRead = iRead;
	}
	AvRevertMmThreadCharacteristics(hTask);
	if (quisk_sound_state.verbose_sound)
		printf("%s: Exit playback thread\n", dev->stream_description);
	CoUninitialize();
	return 0;
}

void quisk_write_wasapi(struct sound_dev * dev, int nSamples, complex double * cSamples, double volume)
{	// Called from Quisk by the sound thread. Write samples to the playdevice callback buffer.
	...
	if (quisk_sound_state.verbose_sound > 1) {
		tm = QuiskTimeSec();
		if (tm - dev->TimerTime0 > 1.000) {
			printf("%s:  Buffer fill %.2f%%\n", dev->stream_description, buffer_fill * 100);
			dev->TimerTime0 = tm;
		}
	}
...
{	// Called from Quisk by the sound thread. Read samples from the sound card capture buffer.
	...
	while (1) {	// Get the available data
		...
		if (hr != S_OK) {	// Error
			printf("%s:  Error processing buffer\n", dev->stream_description);
			return nSamples;
		}
		...
		if (FAILED(IAudioCaptureClient_ReleaseBuffer(DD->pCaptureClient, numFramesAvailable))) {
			printf("%s:  Failure to release buffer\n", dev->stream_description);
			break;
		}
	}
	return nSamples;
...
{	// Called from Quisk by the sound thread. Write samples to the sound card buffer.
	...
	if (FAILED(IAudioClient_GetCurrentPadding(DD->pAudioClient, &numFramesPadding))) {
		printf("%s:  quisk_play_wasapi failed to get padding\n", dev->stream_description);
		return;
	}
	...
	case 1:	// Normal run state
		...
		// Check if play buffer is too full.
		else if (buffer_fill >= 1.00) {
			quisk_sound_state.write_error++;
			dev->dev_error++;
			if (quisk_sound_state.verbose_sound)
				printf ("%s:  Buffer too full\n", dev->stream_description);
			nSamples = 0;
			dev->started = 2;
		}
			...
			nSamples--;
#if DEBUG_IO
			printf("%s:  play_alsa Remove a sample\n", playdev->stream_description);
#endif
		}
		else if(buffer_fill < 0.3 && nSamples >= 2) {
			cSamples[nSamples] = cSamples[nSamples - 1];
			...
			nSamples++;
#if DEBUG_IO
			printf ("%s:  play_alsa Add a sample\n", playdev->stream_description);
#endif
		}
		break;
	case 2:	 // Buffer is too full; wait for it to drain
		if (buffer_fill <= 0.5) {
			dev->started = 1;
			if (quisk_sound_state.verbose_sound)
				printf("%s:  Resume adding samples\n", dev->stream_description);
		}
		else {
			nSamples = 0;
		}
	...
	if (FAILED(IAudioRenderClient_GetBuffer(DD->pRenderClient, nSamples, &pData))) {
		printf("%s:  quisk_play_wasapi failed get the buffer\n", dev->stream_description);
		return;
	}
	switch (dev->sound_format) {
	case Int16:
...
static void open_wasapi_capture(struct sound_dev * dev)
{
	...
	if (SUCCEEDED(IAudioClient_GetDevicePeriod(DD->pAudioClient, &def_period, &min_period))) {
	        if (quisk_sound_state.verbose_sound)
		        printf("  Device default period %d, min period %d microsec\n",
	...
	if (quisk_sound_state.verbose_sound) {
		...
		printf("  Sound device format %s\n", sound_format_names[dev->sound_format]);
		printf("  Number of channels %d, bytes per sample %d\n", dev->num_channels, dev->sample_bytes);
	}
	hnsRequestedDuration = REFTIMES_PER_MILLISEC * 500;
	for ( ;hnsRequestedDuration > REFTIMES_PER_MILLISEC * 50; hnsRequestedDuration -= (REFTIMES_PER_MILLISEC * 50)) {
...
static void open_wasapi_playback(struct sound_dev * dev, int use_callback)
{
	...
	dev->dev_errmsg[0] = 0;
	if (quisk_sound_state.verbose_sound) {
		printf("Opening Wasapi playback device %s\n  Name %s\n  Device name %s\n", dev->stream_description, dev->name, dev->device_name);
	}
	if (pEnumerator == NULL)
		return;
	...
	if (use_callback) {
		...
		if (hr == AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED) {
			if (quisk_sound_state.verbose_sound)
				printf("  Fix for buffer size not aligned\n");
		...
		DD->playbuf_read_reset = 0;
		DD->playbuf_write_reset = 0;
		the_thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) playdevice_callback, dev, 0, &ThreadID);
		if (the_thread == NULL ) {
	...
	if (quisk_sound_state.verbose_sound) {
		...
		if (use_callback)
			printf("  Callback ring buffer size %d frames\n", DD->playbuf_nFrames);
		printf("  Started.\n");
	}
}
...
{  // Open the sound devices and start them. Called from the sound thread.
	...
	if (FAILED(CoCreateInstance(CLSID_MMDEVICEENUMERATOR, NULL, CLSCTX_ALL, IID_IMMDEVICEENUMERATOR, (void**)&pEnumerator))) {
		printf("CoCreateInstance failed in start_sound_wasapi\n");
		pEnumerator = NULL;
		return;
	}
	while (*pCapture) {
...
{	// Return a list of sound device data [pycapt, pyplay]
	...
	if (FAILED(hr)) {
		printf("CoCreateInstance failed in wasapi_sound_devices\n");
		return pylist;
	}
