...
void QuiskMeasureRate(const char * msg, int count, int index)
{  //measure the sample rate for index 0 to 9
	double tm;
	...
	total[index] += count;
	tm = QuiskTimeSec();
	if (tm > time_pr[index] + 1.0) {	// time to print
		time_pr[index] = tm;
		printf("%s count %ld, time %.3lf, rate %.3lf\n", msg, total[index], tm - time0[index], total[index] / (tm - time0[index]));
	}
}
