import os,sys
import importlib
from pygear3.core import Global
import inspect
from pyhelper3.YamlHelper import YamlHelper
import os
import pygear3.global_v as global_v
import asyncio
from pygear3.core.AsyncManager import AsyncManager
class Gear(object):
    __init_path = dict()
    __config_path = None
    __open_server = False
    ...
    async def init(manager):
        ...
        if Gear.__open_server:
            Gear.__init_path['pygear3.worker'] = set()
            Gear.__init_path['pygear3.worker'].add(Gear.get_parent_dir(os.path.dirname(__file__),1)+"\\worker\\")
        ...
        for key in bit_list:
            for work in Global.g_worker_dict[key]:
                task = manager.add_task(func=work.product())
    @staticmethod
    def start():
        loop = asyncio.get_event_loop()
        try:
            # 启动
            global_v.manager = AsyncManager()
            asyncio.ensure_future(Gear.init(global_v.manager))
            # 完成任务就退出
