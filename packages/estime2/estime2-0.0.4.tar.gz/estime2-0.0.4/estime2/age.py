...
class Age():
    ...
    def __init__(
        ...
        * `age`: `int` or `str`. If `int`, it should fall in `[-1, 000+]`,
        ...
        `-2` will return an error; both `100` and `101` will return `100+`.
        If str, one can type either an int like `'94'` (which will be 
        initialized as the age `94`), or `'99+'` where it is then defined 
        as the age `99+`.
        '''
            ...
            if age[-1] == '+':
                ...
                try:
                    ...
                    assert age >= age_min, \
                        ...
                        "but the current value is {0}.".format(age)
                except ValueError:
                    msg =\
                        '{0} in front of "+" cannot be converted ' +\
                        'to int.'.format(age[:-1])
                ...
                try:
                    ...
                    msg =\
                        ...
                        'therefore not convertible to Age.'.format(age)
                    raise ValueError(msg)
        else:
            self.__init__(str(age))

        self.__age = age
        self.__plus = age > get_option('age.max') or has_plus
