...
def plot_CellEMA_sqsum() -> (int, str):
    """
    Test CellEMA with sqsum=1
    ...
    signal.make_wav(sin1_arr, "./CellEMA_sqsum.wav")

    api = synacell.cmodule.SnnAPI
    net = api.new_net()
    net.add_part("id=0,type=CellData,file=./CellEMA_sqsum.wav")
    net.add_part("id=1000,type=SynaBuffer,ciid=0,coid=1")
    net.add_part("id=1,type=CellEMA,alpha=0.02,sqsum=1")
    net.add_part("id=1001,type=SynaBuffer,ciid=0,coid=2")
    net.add_part("id=2,type=CellEMA,alpha=0.002,sqsum=1")
    ...
    fig.suptitle('CellEMA sqsum=1 test')
    # plot 1
    ax[0].plot([i * 1.0 / 16000.0 for i in record[0].pc], record[0].data, '-', label="input signal")
    ax[0].grid(True)
    ax[0].legend()
