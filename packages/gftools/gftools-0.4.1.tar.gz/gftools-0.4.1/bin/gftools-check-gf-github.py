...
def _get_issues_paginate(request_issues, start, end, headers):
  """
...
def _get_issues(request_issues, start, end):
  """
...
def get_issues(request_issues, start, end):
  ...
  if 'link' in request_issues.headers:
    issues = _get_issues_paginate(request_issues, start, end, headers)
  else:
    issues = _get_issues(request_issues, start, end)
  return issues
...
def main():
  ...
  issues = get_issues(request_issues, str(start), str(end))
  print(len(issues['closed_prs']))
