...
def hori_bsl(ID,bsl_all,st_series,outlier_flag=None,writefile=True):
    ...
    #            SVL_TPR
                ...
                print('Calculation of Vertical Presure Difference  for: ' + str(ID[i]) + ' <-> ' + str(ID[j]))
            
                df_prs1_mean = df_bsl['prs1'] - df_bsl['prs1'].mean()
                df_prs2_mean = df_bsl['prs2'] - df_bsl['prs2'].mean()
                
                df_prs1_mean = df_prs1_mean.to_frame()
                df_prs2_mean = df_prs2_mean.to_frame()
                df_prs2_mean.to_csv('gg')
                #print(df_prs1_mean)
                
                df_prs1_mean = df_prs1_mean.rename(columns={"prs1": "prs_mean1"})
                df_prs2_mean = df_prs2_mean.rename(columns={"prs2": "prs_mean2"})
                ...
                df_prs1_newindex = df_prs1_mean.reindex(df_prs2_mean.index).append(df_prs1_mean).sort_index()
                
                prs_interpolate = df_prs1_newindex.interpolate(method ='slinear')
                
                #print(prs_interpolate)
                
                prs_diff = df_prs2_mean.sub(prs_interpolate).rename(columns={"prs_mean2": "prs_diff"}).dropna()
                
                #prs_diff = pd.DataFrame({"prs_diff": df_prs2_mean.subtract(prs_interpolate).dropna()})
                
                prs_diff.to_csv('PRS_DIFF', header=True, date_format=GMT_DATEFORMAT)
                
                result = pd.concat([df_bsl, prs_diff], axis=1)
                
                #cal_bsl_all.append(result)
